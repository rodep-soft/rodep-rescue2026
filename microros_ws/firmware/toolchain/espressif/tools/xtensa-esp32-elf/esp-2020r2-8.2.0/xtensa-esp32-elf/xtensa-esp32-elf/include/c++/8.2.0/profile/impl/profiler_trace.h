...
inline std::size_t __max_mem() { return _GLIBCXX_PROFILE_DATA(_S_max_mem); }
...
template <typename __object_info, typename __stack_info>
__object_info* __trace_base<__object_info, __stack_info>::__add_object(
    __stack_t __stack) {
...
template <typename __object_info, typename __stack_info>
inline void __trace_report(__trace_base<__object_info, __stack_info>* __cont,
...
inline std::size_t __env_to_size_t(const char* __env_var,
...
inline FILE* __open_output_file(const char* __extension) {
    ...
    __builtin_memcpy(__file_name, _GLIBCXX_PROFILE_DATA(_S_trace_file_name),
                     __root_len);
...
struct __cost_factor_setter {
    void operator()(__cost_factor* __factor) {
        ...
        if (__env_value) __factor->__value = std::atof(__env_value);
    }
};
...
inline bool __profcxx_init() {
    if (__is_invalid()) __profcxx_init_unconditional();
