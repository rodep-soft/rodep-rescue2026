...
class _Safe_local_iterator : private _Iterator,
   ...
   public:
    ...
    typename __gnu_cxx::__conditional_type<
        std::__are_same<_Const_local_iterator, _Safe_local_iterator>::__value,
...
__get_distance(const _Safe_local_iterator<_Iterator, _Sequence>& __first,
    ...
    if (__first.base() == __last.base()) return {0, __dp_exact};

    if (__first._M_is_begin()) {
        if (__last._M_is_end())
            return {__first._M_get_sequence()->bucket_size(__first.bucket()),
                    __dp_exact};
    ...
    if (__last._M_is_begin()) return {-1, __dp_sign};

    if (__last._M_is_end()) return {1, __dp_sign};
