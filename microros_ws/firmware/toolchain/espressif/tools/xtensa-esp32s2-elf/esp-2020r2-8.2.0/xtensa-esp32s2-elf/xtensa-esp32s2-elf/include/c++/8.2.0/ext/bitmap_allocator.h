...
inline size_t __num_blocks(_AddrPair __ap) {
 ...
 */
template <typename _AddrPair>
inline size_t __num_bitmaps(_AddrPair __ap) {
...
class _Ffit_finder
   ...
   public:
    ...
    size_t* _M_get() const throw() { return _M_pbitmap; }
...
class _Bitmap_counter {
   ...
   public:
    ...
    void _M_reset(long __index = -1) throw() {
        ...
        _M_curr_bmap =
            reinterpret_cast<size_t*>(_M_vbp[_M_curr_index].first) - 1;
    ...
    size_t* _M_get() const throw() { return _M_curr_bmap; }
    ...
    _Index_type _M_offset() const throw() {
        return size_t(bits_per_block) *
               ((reinterpret_cast<size_t*>(this->_M_base()) - _M_curr_bmap) -
                1);
    }
   ...
   public:
    ...
    pointer _M_allocate_single_object() _GLIBCXX_THROW(std::bad_alloc) {
        ...
        size_t* __puse_count =
            ...
            (__detail::__num_bitmaps(
                 _S_mem_blocks[_S_last_request._M_where()]) +
             1);
