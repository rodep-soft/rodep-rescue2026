...
class _Safe_iterator : private _Iterator, public _Safe_iterator_base {
   ...
   public:
     ...
     */
    _Safe_iterator(const _Iterator& __i,
    ...
    _Safe_iterator& operator+=(const difference_type& __n) _GLIBCXX_NOEXCEPT {
        _GLIBCXX_DEBUG_VERIFY(
    ...
    typename __gnu_cxx::__conditional_type<
        std::__are_same<_Const_iterator, _Safe_iterator>::__value,
...
template <typename _Iterator, typename _Sequence>
inline typename _Safe_iterator<_Iterator, _Sequence>::difference_type operator-(
    const _Safe_iterator<_Iterator, _Sequence>& __lhs,
    const _Safe_iterator<_Iterator, _Sequence>& __rhs) _GLIBCXX_NOEXCEPT {
    _GLIBCXX_DEBUG_VERIFY(!__lhs._M_singular() && !__rhs._M_singular(),
                          _M_message(__msg_distance_bad)
                              ._M_iterator(__lhs, "lhs")
                              ._M_iterator(__rhs, "rhs"));
...
template <typename _Iterator, typename _Sequence>
inline _Safe_iterator<_Iterator, _Sequence> operator+(
...
template <typename _Iterator, typename _Sequence>
inline bool __check_dereferenceable(
...
inline typename _Distance_traits<_Iterator>::__type __get_distance(
    ...
    if (__first.base() == __last.base()) return std::make_pair(0, __dp_exact);

    if (__first._M_is_before_begin()) {
        if (__last._M_is_begin()) return std::make_pair(1, __dp_exact);

        return std::make_pair(1, __dp_sign);
    }

    if (__first._M_is_begin()) {
        if (__last._M_is_before_begin()) return std::make_pair(-1, __dp_exact);
    ...
    if (__last._M_is_end()) return std::make_pair(1, __dp_sign);
...
inline typename _Distance_traits<_Iterator>::__type __get_distance_from_begin(
    ...
    if (__it._M_is_begin()) return std::make_pair(0, __dp_exact);

    if (__it._M_is_end()) return _SeqTraits::_S_size(*__it._M_get_sequence());

    typename _Distance_traits<_Iterator>::__type __res =
        __get_distance(__it._M_get_sequence()->_M_base().begin(), __it.base());

    if (__res.second == __dp_equality) return std::make_pair(1, __dp_sign);
...
template <typename _Iterator, typename _Sequence>
inline typename _Distance_traits<_Iterator>::__type __get_distance_to_end(
    const _Safe_iterator<_Iterator, _Sequence>& __it) {
    typedef _Sequence_traits<_Sequence> _SeqTraits;

    // No need to consider before_begin as this function is only used in
    // _M_can_advance which won't be used for forward_list iterators.
    if (__it._M_is_begin()) return _SeqTraits::_S_size(*__it._M_get_sequence());

    if (__it._M_is_end()) return std::make_pair(0, __dp_exact);
...
#else
template <typename _Iterator, typename _Sequence>
_Iterator __base(const _Safe_iterator<_Iterator, _Sequence>& __it,
                 std::random_access_iterator_tag) {
    return __it.base();
}

template <typename _Iterator, typename _Sequence>
const _Safe_iterator<_Iterator, _Sequence>& __base(
    const _Safe_iterator<_Iterator, _Sequence>& __it, std::input_iterator_tag) {
    return __it;
}

template <typename _Iterator, typename _Sequence>
auto __base(const _Safe_iterator<_Iterator, _Sequence>& __it)
