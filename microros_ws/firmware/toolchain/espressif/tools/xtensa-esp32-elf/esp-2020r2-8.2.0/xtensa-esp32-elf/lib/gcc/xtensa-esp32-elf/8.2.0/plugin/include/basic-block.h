...
static inline bool bb_has_eh_pred(basic_block bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->preds) {
        if (e->flags & EDGE_EH) return true;
    }
    return false;
}
...
static inline bool bb_has_abnormal_pred(basic_block bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->preds) {
        if (e->flags & EDGE_ABNORMAL) return true;
    }
    return false;
}
...
static inline bool has_abnormal_or_eh_outgoing_edge_p(basic_block bb) {
    ...
    if (e->flags & (EDGE_ABNORMAL | EDGE_EH)) return true;
...
static inline bool has_abnormal_call_or_eh_pred_edge_p(basic_block bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->preds)
    if (e->flags & (EDGE_ABNORMAL_CALL | EDGE_EH)) return true;
