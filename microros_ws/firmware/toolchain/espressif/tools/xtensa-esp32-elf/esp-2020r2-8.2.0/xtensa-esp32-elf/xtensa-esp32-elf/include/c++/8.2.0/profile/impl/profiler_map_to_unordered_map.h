...
inline int __log2(std::size_t __size) {
    for (int __bit_count = sizeof(std::size_t) - 1; __bit_count >= 0;
         --__bit_count)
        if ((2 << __bit_count) & __size) return __bit_count;
    return 0;
}
   ...
   public:
    ...
    void __write(FILE* __f) const {
        std::fprintf(__f, "%Zu %Zu %Zu %Zu %.0f %.0f\n", _M_insert, _M_erase,
                     _M_find, _M_iterate, _M_map_cost, _M_umap_cost);
    }

    float __magnitude() const { return _M_map_cost - _M_umap_cost; }

    std::string __advice() const { return "prefer an unordered container"; }
...
inline void __trace_map_to_unordered_map_insert(__map2umap_info* __info,
                                                std::size_t __size,
                                                std::size_t __count) {
    if (!__info) return;
...
inline void __trace_map_to_unordered_map_erase(__map2umap_info* __info,
                                               std::size_t __size,
                                               std::size_t __count) {
    if (!__info) return;
...
inline void __trace_map_to_unordered_map_find(__map2umap_info* __info,
                                              std::size_t __size) {
    if (!__info) return;
