...
class predicate {
   public:
    ...
    /* Special condition code we use to represent test that operand is not
       changed across invocation of the function.  When operand IS_NOT_CONSTANT
       it is always CHANGED, however i.e. loop invariants can be NOT_CHANGED
       given percentage of executions even when they are not compile time
       constants.  */
     ...
     */
    inline predicate(predicate*) { gcc_unreachable(); }
    ...
    inline bool operator==(const predicate& p2) const {
        ...
        for (i = 0; m_clause[i]; i++) {
            ...
            if (m_clause[i] != p2.m_clause[i]) return false;
        }
    ...
    inline bool operator==(const bool cond) const {
        if (cond) return !m_clause[0];
        if (m_clause[0] == (1 << false_condition)) {
            gcc_checking_assert(!m_clause[1] &&
                                m_clause[0] == 1 << false_condition);
            return true;
        }
        return false;
    }

    inline bool operator!=(const predicate& p2) const { return !(*this == p2); }
    ...
    predicate remap_after_inlining(struct ipa_fn_summary*,
                                   struct ipa_fn_summary*, vec<int>, vec<int>,
