...
RoboclawDriver::RoboclawDriver(std::string port, uint8_t address)
  ...
  try {
    ...
    RCLCPP_INFO(logger_, "Roboclaw初期化完了: ポート=%s, アドレス=0x%02X", port_.c_str(), address_);
  } catch (const boost::system::system_error& e) {
    RCLCPP_ERROR(logger_, "シリアルポート初期化失敗: %s", e.what());
    throw std::runtime_error("シリアルポート初期化失敗: " + std::string(e.what()));
  }
}
...
void RoboclawDriver::sendCommand(ByteBuffer command, Callback callback) {
  ...
  boost::asio::async_write(
      ...
      [this, buffer, callback = std::move(callback)](const boost::system::error_code& ec,
                                                     std::size_t) {
        if (ec) {
          RCLCPP_ERROR(logger_, "書き込みエラー: %s", ec.message().c_str());
          callback(false);
          return;
        }
        ...
        // タイムアウト設定
        timer->async_wait([this, response](const boost::system::error_code& ec) {
          if (!ec) {
        ...
        boost::asio::async_read(
            ...
            [this, response, timer, callback](const boost::system::error_code& ec, std::size_t) {
              timer->cancel();

              if (ec) {
                RCLCPP_ERROR(logger_, "読み取りエラー: %s", ec.message().c_str());
                callback(false);
                return;
              }
              ...
              if (!success) {
                RCLCPP_WARN(logger_, "無効な応答: 0x%02X (期待値: 0xFF)", *response);
              }
              RCLCPP_DEBUG(logger_, "応答受信: 0x%02X %s", *response, success ? "✓" : "✗");
              callback(success);
            });
      });
}
...
void RoboclawDriver::sendCommandWithResponse(
  ...
  boost::asio::async_write(
      ...
      [this, buffer, response_size, callback = std::move(callback)](
        ...
        boost::asio::async_read(serial_port_, boost::asio::buffer(*response),
...
void RoboclawDriver::setVelocity(Motor motor, double counts_per_sec, Callback callback) {
  const auto cmd = getVelocityCommand(motor);
  auto command = buildCommand(cmd, static_cast<int32_t>(counts_per_sec));
  RCLCPP_DEBUG(logger_, "速度設定: M%d -> %.0f counts/sec", static_cast<int>(motor),
               counts_per_sec);
  sendCommand(std::move(command), std::move(callback));
}

void RoboclawDriver::setPID(Motor motor, const PIDConstants& pid, Callback callback) {
  const auto cmd = getPIDCommand(motor);
  auto command = buildCommand(cmd, pid);
  RCLCPP_DEBUG(logger_, "PID設定: M%d -> P=%.3f I=%.3f D=%.3f QPPS=%d", static_cast<int>(motor),
               pid.p, pid.i, pid.d, pid.qpps);
  sendCommand(std::move(command), std::move(callback));
}
...
void RoboclawDriver::readEncoder(Motor motor, EncoderCallback callback) {
  ...
  sendCommandWithResponse(
      std::move(command), 7,
      [this, motor, callback = std::move(callback)](std::optional<ByteBuffer> response) {
        if (!response || response->size() != 7) {
          RCLCPP_ERROR(logger_, "M%d: エンコーダ読み取り失敗 (サイズ不正)",
        ...
        const uint16_t received_crc = (static_cast<uint16_t>((*response)[5]) << 8) | (*response)[6];
        const uint16_t calculated_crc = calculateCRC(std::span(response->data(), 5));

        if (received_crc != calculated_crc) {
          RCLCPP_ERROR(logger_, "M%d: CRCエラー (期待: 0x%04X, 受信: 0x%04X)",
        ...
        RCLCPP_DEBUG(logger_, "M%d: エンコーダ=%d counts, status=0x%02X", static_cast<int>(motor),
...
RoboclawDriver::ByteBuffer RoboclawDriver::buildCommand(Command cmd, int32_t value) const {
  ByteBuffer data = {address_, static_cast<uint8_t>(cmd)};
...
int32_t RoboclawDriver::extractInt32BE(std::span<const uint8_t> data, size_t offset) {
  return (static_cast<int32_t>(data[offset]) << 24) |
         (static_cast<int32_t>(data[offset + 1]) << 16) |
         (static_cast<int32_t>(data[offset + 2]) << 8) | static_cast<int32_t>(data[offset + 3]);
}
