...
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::to_shwi(
...
template <typename storage>
template <typename T>
inline generic_wide_int<storage>& generic_wide_int<storage>::operator=(
    const T& x) {
    storage::operator=(x);
    return *this;
}
...
void generic_wide_int<storage>::dump() const {
    ...
    unsigned int precision = this->get_precision();
    fprintf(stderr, "[");
    if (len * HOST_BITS_PER_WIDE_INT < precision) fprintf(stderr, "...,");
...
inline const HOST_WIDE_INT* wide_int_storage::get_val() const { return val; }

inline unsigned int wide_int_storage::get_len() const { return len; }

inline HOST_WIDE_INT* wide_int_storage::write_val() { return val; }
...
template <typename T1, typename T2>
inline wide_int wi::int_traits<wide_int_storage>::get_binary_result(
...
inline HOST_WIDE_INT* trailing_wide_int_storage::write_val() { return m_val; }
...
template <typename T>
inline trailing_wide_int_storage& trailing_wide_int_storage::operator=(
    const T& x) {
...
inline unsigned int wi::int_traits<wi::hwi_with_prec>::get_precision(
    const wi::hwi_with_prec& x) {
    return x.precision;
}

inline wi::storage_ref wi::int_traits<wi::hwi_with_prec>::decompose(
    HOST_WIDE_INT* scratch, unsigned int precision,
    const wi::hwi_with_prec& x) {
...
inline bool wi::fits_uhwi_p(const T& x) {
    WIDE_INT_REF_FOR(T) xi(x);
    if (xi.precision <= HOST_BITS_PER_WIDE_INT) return true;
    if (xi.len == 1) return xi.slow() >= 0;
...
inline WI_UNARY_RESULT(T) wi::bit_not(const T& x) {
    ...
    for (unsigned int i = 0; i < xi.len; ++i) val[i] = ~xi.val[i];
...
template <typename T>
inline WI_UNARY_RESULT(T) wi::neg(const T& x, bool* overflow) {
...
inline WI_BINARY_RESULT(T1, T2) wi::add(const T1& x, const T2& y, signop sgn,
    ...
    if (precision <= HOST_BITS_PER_WIDE_INT) {
        ...
        if (sgn == SIGNED)
            *overflow =
                (((resultl ^ xl) & (resultl ^ yl)) >> (precision - 1)) & 1;
        else
...
inline unsigned HOST_WIDE_INT wi::extract_uhwi(const T& x, unsigned int bitpos,
    ...
    if (precision < bitpos + width) precision = bitpos + width;
    WIDE_INT_REF_FOR(T) xi(x, precision);
