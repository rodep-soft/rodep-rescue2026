...
class multiset : public _GLIBCXX_STD_C::multiset<_Key, _Compare, _Allocator>,
   ...
   public:
    ...
    multiset(multiset&& __x,
             const allocator_type& __a) noexcept(noexcept(_Base(std::move(__x),
                                                                __a)))
    ...
    void insert(_InputIterator __first, _InputIterator __last) {
        for (; __first != __last; ++__first) insert(*__first);
    }
    ...
    iterator erase(const_iterator __first, const_iterator __last) {
        if (__first != __last) {
            iterator __ret;
            for (; __first != __last;) __ret = erase(__first++);
            return __ret;
        } else
    ...
    void erase(iterator __first, iterator __last) {
        for (; __first != __last;) erase(__first++);
    }
#endif
