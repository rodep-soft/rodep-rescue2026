...
struct symbol_compare_hash : nofree_ptr_hash<symbol_compare_collection> {
    static hashval_t hash(value_type v) {
        ...
        for (unsigned i = 0; i < v->m_interposables.length(); i++)
            hstate.add_int(
                v->m_interposables[i]->ultimate_alias_target()->order);
...
class sem_item {
   public:
    ...
    virtual bool equals_wpa(
        sem_item* item, hash_map<symtab_node*, sem_item*>& ignored_nodes) = 0;
   ...
   protected:
    ...
    bool compare_symbol_references(
        hash_map<symtab_node*, sem_item*>& ignored_nodes, symtab_node* n1,
...
class sem_variable : public sem_item {
   public:
    ...
    inline virtual void init(void) { decl = get_node()->decl; }
...
class sem_item_optimizer {
   ...
   private:
    ...
    void do_congruence_step_for_index(congruence_class* cls,
                                      unsigned int index);
