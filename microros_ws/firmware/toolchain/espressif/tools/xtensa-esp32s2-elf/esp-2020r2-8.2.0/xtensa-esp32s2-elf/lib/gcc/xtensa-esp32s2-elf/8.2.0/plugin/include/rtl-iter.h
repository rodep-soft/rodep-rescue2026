...
inline generic_subrtx_iterator<T>::array_type::~array_type() {
    if (__builtin_expect(heap != 0, false)) free_array(*this);
}
...
template <typename T>
inline bool generic_subrtx_iterator<T>::at_end() const {
    return m_done;
}
...
template <typename T>
inline void generic_subrtx_iterator<T>::next() {
    if (m_substitute) {
        m_substitute = false;
        m_skip = false;
        return;
        ...
        if (__builtin_expect(x != 0, true)) {
            ...
            if (count > 0) {
               ...
               the sequence might overrun M_BASE.  */
                ...
                if (count > 0) {
                    m_end += count;
                    if (m_end > LOCAL_ELEMS) m_base = m_array.heap->address();
...
template <typename T>
inline void generic_subrtx_iterator<T>::skip_subrtxes() {
    m_skip = true;
}
