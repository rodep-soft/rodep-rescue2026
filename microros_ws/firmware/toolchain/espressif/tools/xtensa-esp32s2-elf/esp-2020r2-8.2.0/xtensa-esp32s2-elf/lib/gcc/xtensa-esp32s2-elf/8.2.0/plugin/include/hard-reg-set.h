...
#define CLEAR_HARD_REG_SET(TO)                                    \
    do {                                                          \
        HARD_REG_ELT_TYPE* scan_tp_ = (TO);                       \
        int i;                                                    \
        for (i = 0; i < HARD_REG_SET_LONGS; i++) *scan_tp_++ = 0; \
    } while (0)
...
#define SET_HARD_REG_SET(TO)                                       \
    do {                                                           \
        HARD_REG_ELT_TYPE* scan_tp_ = (TO);                        \
        int i;                                                     \
        for (i = 0; i < HARD_REG_SET_LONGS; i++) *scan_tp_++ = -1; \
    } while (0)
...
#define COPY_HARD_REG_SET(TO, FROM)                                         \
    do {                                                                    \
        ...
        const HARD_REG_ELT_TYPE* scan_fp_ = (FROM);                         \
        int i;                                                              \
        for (i = 0; i < HARD_REG_SET_LONGS; i++) *scan_tp_++ = *scan_fp_++; \
    } while (0)
...
static inline bool hard_reg_set_iter_set(hard_reg_set_iterator* iter,
    ...
    while (1) {
        ...
        if (iter->word_no >= iter->length) return false;
