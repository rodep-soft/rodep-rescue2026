...
struct GTY(()) size_time_entry {
    ...
    /* Predicate for value to be constant and optimized out in a specialized
       copy. When deciding on specialization this makes it possible to see how
       much the executed code paths will simplify.  */
...
struct GTY(()) ipa_fn_summary {
    ...
    ipa_fn_summary()
        : estimated_self_stack_size(0),
          ...
          stack_frame_offset(false),
          time(0),
          size(0),
          conds(NULL),
          size_time_table(NULL),
   ...
   public:
    ...
    static ipa_fn_summary_t* create_ggc(symbol_table* symtab) {
        struct ipa_fn_summary_t* summary =
            new (ggc_alloc<ipa_fn_summary_t>()) ipa_fn_summary_t(symtab, true);
...
void estimate_ipcp_clone_size_and_time(struct cgraph_node*, vec<tree>,
                                       ...
                                       vec<ipa_agg_jump_function_p>, int*,
                                       sreal*, sreal*, ipa_hints*);
void ipa_merge_fn_summary_after_inlining(struct cgraph_edge* edge);
void ipa_update_overall_fn_summary(struct cgraph_node* node);
void compute_fn_summary(struct cgraph_node*, bool);

void evaluate_properties_for_edge(
    struct cgraph_edge* e, bool inline_p, clause_t* clause_ptr,
    clause_t* nonspec_clause_ptr, vec<tree>* known_vals_ptr,
    vec<ipa_polymorphic_call_context>* known_contexts_ptr,
    vec<ipa_agg_jump_function_p>*);
void estimate_node_size_and_time(
    struct cgraph_node* node, clause_t possible_truths,
    clause_t nonspec_possible_truths, vec<tree> known_vals,
    vec<ipa_polymorphic_call_context>, vec<ipa_agg_jump_function_p> known_aggs,
    int* ret_size, int* ret_min_size, sreal* ret_time,
    sreal* ret_nonspecialized_time, ipa_hints* ret_hints,
