...
class hash_table {
   ...
   private:
    ...
    friend void gt_pch_nx(hash_set<T, U>*, gt_pointer_operator, void*);
    template <typename T>
    ...
    void expand();
    static bool is_deleted(value_type& v) { return Descriptor::is_deleted(v); }

    static bool is_empty(value_type& v) { return Descriptor::is_empty(v); }

    static void mark_deleted(value_type& v) { Descriptor::mark_deleted(v); }
...
hash_table<Descriptor, Allocator>::hash_table(
    size_t size, bool ggc, bool gather_mem_stats,
...
template <typename Descriptor, template <typename Type> class Allocator>
hash_table<Descriptor, Allocator>::hash_table(
    const hash_table& h, bool ggc, bool gather_mem_stats,
    mem_alloc_origin origin MEM_STAT_DECL)
    : m_n_elements(h.m_n_elements),
      m_n_deleted(h.m_n_deleted),
      m_searches(0),
      m_collisions(0),
      m_ggc(ggc),
      m_gather_mem_stats(gather_mem_stats) {
...
hash_table<Descriptor, Allocator>::alloc_entries(size_t n MEM_STAT_DECL) const {
    ...
    for (size_t i = 0; i < n; i++) mark_empty(nentries[i]);
...
template <typename Descriptor, template <typename Type> class Allocator>
inline bool hash_table<Descriptor, Allocator>::too_empty_p(unsigned int elts) {
...
template <typename Descriptor, template <typename Type> class Allocator>
void hash_table<Descriptor, Allocator>::empty_slow() {
    size_t size = m_size;
    size_t nsize = size;
    ...
    if (nsize != size) {
        ...
        for (; size; ++entries, --size) *entries = value_type();
#else
...
template <typename Descriptor, template <typename Type> class Allocator>
typename hash_table<Descriptor, Allocator>::value_type&
hash_table<Descriptor, Allocator>::find_with_hash(
...
hash_table<Descriptor, Allocator>::find_slot_with_hash(
    ...
    if (insert == INSERT && m_size * 3 <= m_n_elements * 4) expand();
    ...
    for (;;) {
        ...
        if (is_empty(*entry))
            ...
            if (!first_deleted_slot) first_deleted_slot = &m_entries[index];
        } else if (Descriptor::equal(*entry, comparable))
            return &m_entries[index];
    }

empty_entry:
    if (insert == NO_INSERT) return NULL;
...
template <typename Descriptor, template <typename Type> class Allocator>
void hash_table<Descriptor, Allocator>::remove_elt_with_hash(
    const compare_type& comparable, hashval_t hash) {
...
void hash_table<Descriptor, Allocator>::traverse_noresize(Argument argument) {
    ...
    do {
        ...
        if (!is_empty(x) && !is_deleted(x))
            if (!Callback(slot, argument)) break;
    } while (++slot < limit);
}
...
void hash_table<Descriptor, Allocator>::traverse(Argument argument) {
    if (too_empty_p(elements())) expand();
...
void hash_table<Descriptor, Allocator>::iterator::slide() {
    for (; m_slot < m_limit; ++m_slot) {
        value_type& x = *m_slot;
        if (!is_empty(x) && !is_deleted(x)) return;
    }
    m_slot = NULL;
    m_limit = NULL;
}
...
static inline void gt_ggc_mx(hash_table<E>* h) {
    ...
    if (!ggc_test_and_set_mark(h->m_entries)) return;
