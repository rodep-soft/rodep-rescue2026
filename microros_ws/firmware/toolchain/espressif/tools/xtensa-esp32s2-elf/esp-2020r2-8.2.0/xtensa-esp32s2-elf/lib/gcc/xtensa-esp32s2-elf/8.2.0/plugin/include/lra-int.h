...
static void inline lra_change_class(int regno, enum reg_class new_class,
    ...
    if (lra_dump_file != NULL && nl_p) fprintf(lra_dump_file, "\n");
}
...
static inline void lra_update_reg_val_offset(int val, poly_int64 incr) {
    ...
    for (i = FIRST_PSEUDO_REGISTER; i < max_reg_num(); i++) {
        if (lra_reg_info[i].val == val) lra_reg_info[i].offset += incr;
    }
}
