   ...
   public:
    ...
    enum reachability {
        ...
        /* Identical to REACHABLE_BLOCKS, but the initial state of
           EDGE_EXECUTABLE will instead be preserved in the ctor, allowing for
           information about non-executable edges to be merged in from an
           earlier analysis (and potentially for additional edges to be marked
           as non-executable).  */
    ...
    /* You can provide a mapping of basic-block index to RPO if you
       ...
       walked in RPO order.  */
    dom_walker(cdi_direction direction, enum reachability,
               int* bb_index_to_rpo);
