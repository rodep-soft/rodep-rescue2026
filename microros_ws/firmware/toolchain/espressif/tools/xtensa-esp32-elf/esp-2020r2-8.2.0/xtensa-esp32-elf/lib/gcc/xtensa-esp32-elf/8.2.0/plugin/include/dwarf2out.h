...
struct GTY(()) dw_val_node {
    ...
    union dw_val_struct_union {
        ...
        struct dwarf_file_data* GTY((tag("dw_val_class_file_implicit")))
            val_file_implicit;
...
extern struct dw_loc_descr_node* build_cfa_aligned_loc(dw_cfa_location*,
                                                       poly_int64,
                                                       HOST_WIDE_INT);
...
struct fixed_point_type_info {
    /* A scale factor is the value one has to multiply with physical data in
       order to get the fixed point logical data.  The DWARF standard enables
       one to encode it in three ways.  */
