...
struct _bnd {
    ...
    /* Deps context at this boundary.  As long as we have one boundary per
       fence, this is just a pointer to the same deps context as in the
       corresponding fence.  */
    deps_t dc;
};
...
static inline _list_t _list_alloc(void) { return sched_lists_pool.allocate(); }
...
static inline void _list_iter_start(_list_iterator* ip, _list_t* lp,
                                    bool can_remove_p) {
    ip->lp = lp;
...
static inline bool ilist_is_in_p(ilist_t l, insn_t insn) {
    while (l) {
        if (ILIST_INSN(l) == insn) return true;
        l = ILIST_NEXT(l);
    }
...
static inline bool inner_loop_header_p(basic_block bb) {
    ...
    if (!current_loop_nest) return false;

    if (bb == EXIT_BLOCK_PTR_FOR_FN(cfun)) return false;

    inner_loop = bb->loop_father;
    if (inner_loop == current_loop_nest) return false;
...
static inline vec<edge> get_loop_exit_edges_unique_dests(
    ...
    for (exit = loop->exits->next; exit->e; exit = exit->next) {
        ...
        if (!was_dest) edges.safe_push(exit->e);
    }
    return edges;
}
...
static bool sel_bb_empty_or_nop_p(basic_block bb) {
    ...
    if (first == NULL_RTX) return true;

    if (!INSN_NOP_P(first)) return false;
...
/* We need to return a succ_iterator to avoid 'unitialized' warning
   during bootstrap.  */
static inline succ_iterator _succ_iter_start(insn_t* succp, insn_t insn,
                                             int flags) {
    succ_iterator i;
...
static inline bool _succ_iter_cond(succ_iterator* ip, insn_t* succp,
                                   insn_t insn,
    ...
    if (!ip->bb_end) {
        ...
        if (*succp != NULL || (ip->flags & SUCCS_NORMAL) == 0) return false;
        ...
        *succp = NEXT_INSN(insn);
        ...
        while (1) {
            ...
            if (ip->loop_exits.exists()) {
                ...
                if (!e_tmp) ip->loop_exits.release();
            }
           ...
           do one more iteration to fetch an edge from these exits.  */
            if (ip->loop_exits.exists()) continue;
...
static inline void _succ_iter_next(succ_iterator* ip) {
    ...
    if (ip->bb_end && ip->e1 && !ip->loop_exits.exists()) ei_next(&(ip->ei));
}
...
static inline bool _eligible_successor_edge_p(edge e1, succ_iterator* ip) {
    ...
    if (src_outside_rgn) {
        ...
        if (flags & SUCCS_OUT) return false;
    }
    ...
    while (1) {
        if (!sel_bb_empty_p(bb)) {
            ...
            if (!sel_bb_empty_or_nop_p(bb)) break;

            ne = EDGE_SUCC(bb, 0);
            nbb = ne->dest;

            if (!in_current_region_p(nbb) && !(flags & SUCCS_OUT)) break;
        ...
        if (!in_current_region_p(bb) && !(flags & SUCCS_OUT)) return false;
...
#define FOR_EACH_SUCC_1(SUCC, ITER, INSN, FLAGS)                               \
    for ((ITER) = _succ_iter_start(&(SUCC), (INSN), (FLAGS)); _succ_iter_cond( \
             &(ITER), &(SUCC), (INSN), _eligible_successor_edge_p);            \
