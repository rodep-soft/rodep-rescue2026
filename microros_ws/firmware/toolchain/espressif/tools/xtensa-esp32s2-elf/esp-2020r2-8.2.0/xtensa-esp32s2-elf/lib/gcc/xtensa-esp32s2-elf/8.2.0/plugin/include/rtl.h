...
inline unsigned int& INSN_LOCATION(rtx_insn* insn) { return XUINT(insn, 4); }
...
static inline void set_regno_raw(rtx x, unsigned int regno,
                                 unsigned int nregs) {
...
static inline bool always_void_p(enum rtx_code code) { return code == SET; }
...
inline unsigned int wi::int_traits<rtx_mode_t>::get_precision(
    const rtx_mode_t& x) {
...
inline wi::rtx_to_poly_wide_ref wi::to_poly_wide(const_rtx x,
                                                 machine_mode mode) {
    if (CONST_POLY_INT_P(x)) return const_poly_int_value(x);
...
inline bool poly_int_rtx_p(const_rtx x, poly_int64_pod* res) {
    ...
    if (CONST_POLY_INT_P(x)) {
        for (unsigned int i = 0; i < NUM_POLY_INT_COEFFS; ++i)
            if (!wi::fits_shwi_p(CONST_POLY_INT_COEFFS(x)[i])) return false;
        for (unsigned int i = 0; i < NUM_POLY_INT_COEFFS; ++i)
            res->coeffs[i] = CONST_POLY_INT_COEFFS(x)[i].to_shwi();
        return true;
    }
...
inline T unwrap_const_vec_duplicate(T x) {
    if (const_vec_duplicate_p(x)) x = CONST_VECTOR_ELT(x, 0);
    return x;
}
...
inline bool partial_subreg_p(const_rtx x) {
    if (GET_CODE(x) != SUBREG) return false;
...
inline rtx single_set(const rtx_insn* insn) {
    if (!INSN_P(insn)) return NULL_RTX;

    if (GET_CODE(PATTERN(insn)) == SET) return PATTERN(insn);
...
static inline const struct mem_attrs* get_mem_attrs(const_rtx x) {
    ...
    attrs = MEM_ATTRS(x);
    if (!attrs) attrs = mode_mem_attrs[(int)GET_MODE(x)];
    return attrs;
}
#endif
...
/* In expr.c */
extern rtx move_by_pieces(rtx, rtx, unsigned HOST_WIDE_INT, unsigned int, int);
