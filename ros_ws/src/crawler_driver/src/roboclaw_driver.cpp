    ...
    boost::asio::async_write(
        ...
        (const boost::system::error_code& ec, std::size_t) {
            ...
            boost::asio::async_read(
                serial_port_,
                boost::asio::buffer(response.get(), 1),
                ...
                (const boost::system::error_code& ec, std::size_t) {
                    ...
                    RCLCPP_DEBUG(logger_, "応答受信: 0x%02X %s", 
                                *response, success ? "✓" : "✗");
                    callback(success);
                });
        });
...
void RoboclawDriver::readEncoder(Motor motor, EncoderCallback callback) {
    ...
    // マニュアル仕様: 応答フォーマット
    // Encoder Value (4 bytes, Big Endian) + Status (1 byte) + CRC (2 bytes) = 7 bytes
    sendCommandWithResponse(std::move(command), 7,
        [this, motor, callback = std::move(callback)]
        (std::optional<ByteBuffer> response) {
            ...
            const uint16_t received_crc = (static_cast<uint16_t>((*response)[5]) << 8) | 
                                         (*response)[6];
            const uint16_t calculated_crc = calculateCRC(
                std::span(response->data(), 5));
