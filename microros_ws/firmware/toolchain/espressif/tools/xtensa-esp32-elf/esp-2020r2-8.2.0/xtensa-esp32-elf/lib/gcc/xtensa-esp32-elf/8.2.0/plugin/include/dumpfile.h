...
#define TDF_DETAILS                           \
    (1 << 3) /* show more detailed info about \
    each pass */
#define TDF_STATS                                            \
    (1 << 4)                /* dump various statistics about \
                   each pass */
...
#define TDF_MEMSYMS                              \
    (1 << 11) /* display memory symbols in expr. \
     Implies TDF_VOPS.  */

#define TDF_RHS_ONLY                                             \
    (1 << 12)                 /* a flag to only print the RHS of \
                 a gimple stmt.  */
#define TDF_ASMNAME (1 << 13) /* display asm names of decls  */
#define TDF_EH                                                        \
    (1 << 14)                             /* display EH region number \
...
#define MSG_ALL (MSG_OPTIMIZED_LOCATIONS | MSG_MISSED_OPTIMIZATION | MSG_NOTE)
...
extern void dump_printf_loc(dump_flags_t, source_location, const char*,
                            ...) ATTRIBUTE_PRINTF_3;
...
static inline bool dump_enabled_p(void) { return (dump_file || alt_dump_file); }
...
class dump_manager {
   public:
    ...
    unsigned int dump_register(const char* suffix, const char* swtch,
                               ...
                               int optgroup_flags, bool take_ownership);
    ...
    struct dump_file_info* get_dump_file_info_by_switch(
    ...
    char* get_dump_file_name(struct dump_file_info* dfi) const;
    ...
    FILE* dump_begin(int phase, dump_flags_t* flag_ptr);
   ...
   private:
    ...
    int opt_info_enable_passes(int optgroup_flags, dump_flags_t flags,
                               const char* filename);
