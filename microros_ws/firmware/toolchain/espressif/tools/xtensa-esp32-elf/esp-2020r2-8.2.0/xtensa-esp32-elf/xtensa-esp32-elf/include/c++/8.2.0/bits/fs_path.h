...
class path {
    ...
    using _Path =
        typename std::enable_if<__and_<__not_<is_same<_Tp1, path>>,
                                       __constructible_from<_Tp1, _Tp2>>::value,
                                path>::type;

    template <typename _Source>
    static _Source _S_range_begin(_Source __begin) {
        return __begin;
    }
    ...
    template <typename _Source>
    static __null_terminated _S_range_end(_Source) {
        return {};
    }

    template <typename _CharT, typename _Traits, typename _Alloc>
    static const _CharT* _S_range_begin(
        const basic_string<_CharT, _Traits, _Alloc>& __str) {
        return __str.data();
    }

    template <typename _CharT, typename _Traits, typename _Alloc>
    static const _CharT* _S_range_end(
        const basic_string<_CharT, _Traits, _Alloc>& __str) {
   ...
   public:
    ...
    path(_Source const& __source, format = auto_format)
        : _M_pathname(
              _S_convert(_S_range_begin(__source), _S_range_end(__source))) {
        _M_split_cmpts();
    }
    ...
    template <typename _InputIterator>
    _Path<_InputIterator, _InputIterator>& assign(_InputIterator __first,
    ...
    template <typename _InputIterator>
    _Path<_InputIterator, _InputIterator>& append(_InputIterator __first,
                                                  _InputIterator __last) {
    ...
    template <typename _InputIterator>
    _Path<_InputIterator, _InputIterator>& concat(_InputIterator __first,
                                                  _InputIterator __last) {
   ...
   private:
    ...
    path& _M_append(path __p) {
        if (__p.is_absolute()) operator=(std::move(__p));
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
    ...
    static string_type _S_convert(value_type* __src, __null_terminated) {
        return string_type(__src);
    }

    static string_type _S_convert(const value_type* __src, __null_terminated) {
        return string_type(__src);
    }
    ...
    static string_type _S_convert(_InputIterator __src, __null_terminated) {
        ...
        for (; *__src != _Tp{}; ++__src) __tmp.push_back(*__src);
    ...
    static string_type _S_convert_loc(_InputIterator __src, __null_terminated,
                                      const std::locale& __loc) {
        std::string __tmp;
        while (*__src != '\0') __tmp.push_back(*__src++);
...
basic_istream<_CharT, _Traits>& operator>>(basic_istream<_CharT, _Traits>& __is,
    ...
    using __quoted_string =
        std::__detail::_Quoted_string<decltype(__tmp)&, _CharT>;
    if (__is >> __quoted_string{__tmp, '"', '\\'}) __p = std::move(__tmp);
    return __is;
}
...
template <typename _CharT>
inline path::_Path<_CharT*, _CharT*>& path::operator+=(_CharT __x) {
...
std::basic_string<_CharT, _Traits, _Allocator> path::_S_str_convert(
    ...
    if (__str_codecvt_in(__first, __last, __wstr, __cvt)) return __wstr;
#endif
