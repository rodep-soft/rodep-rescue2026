...
def launch_setup(context, *args, **kwargs):
    ...
    if usb_devices:
        ...
        try:
            ...
            print(
                "[demo.launch.py] Dynamixel hardware detected - using sekirei_moveit.urdf with dynamixel_hardware_interface"
            )
        except PackageNotFoundError:
            print(
                "[demo.launch.py] USB device found but dynamixel_hardware_interface not installed"
            )
    ...
    robot_description = ParameterValue(
        Command(
            [
                ...
                urdf_xacro_path,
                " use_real_hw:=",
                LaunchConfiguration("use_real_hw"),
            ]
        ),
        value_type=str,
    )  # valueはstr型である必要がある
    ...
    moveit_controllers = load_yaml(
        "sekirei_moveit_config", "config/moveit_controllers.yaml"
    )
    ...
    ros2_control_params = load_yaml(
        "sekirei_moveit_config", "config/ros2_controllers.yaml"
    )
    ...
    if moveit_cpp_yaml:
        ...
        if "ompl" in moveit_cpp_yaml and isinstance(moveit_cpp_yaml["ompl"], dict):
            ...
            if "planning_plugins" in ompl_config and isinstance(
                ompl_config["planning_plugins"], list
            ):
                ompl_config["planning_plugins"] = ParameterValue(
                    ompl_config["planning_plugins"]
                )
    ...
    if ompl_planning_yaml:
        ...
        if "planning_plugins" in ompl_planning_yaml and isinstance(
            ompl_planning_yaml["planning_plugins"], list
        ):
            ompl_planning_yaml["planning_plugins"] = ParameterValue(
                ompl_planning_yaml["planning_plugins"]
            )

        # Convert request_adapters list to ParameterValue
        if "request_adapters" in ompl_planning_yaml and isinstance(
            ompl_planning_yaml["request_adapters"], list
        ):
            ompl_planning_yaml["request_adapters"] = ParameterValue(
                ompl_planning_yaml["request_adapters"]
            )

        # Convert response_adapters list to ParameterValue
        if "response_adapters" in ompl_planning_yaml and isinstance(
            ompl_planning_yaml["response_adapters"], list
        ):
            ompl_planning_yaml["response_adapters"] = ParameterValue(
                ompl_planning_yaml["response_adapters"]
            )

        # Convert planner_configs lists in group-specific config
        if "sekirei_arm" in ompl_planning_yaml and isinstance(
            ompl_planning_yaml["sekirei_arm"], dict
        ):
            if "planner_configs" in ompl_planning_yaml["sekirei_arm"]:
                if isinstance(
                    ompl_planning_yaml["sekirei_arm"]["planner_configs"], list
                ):
            ...
            while time.time() < deadline:
                ...
                if all(s in services for s in required):
                    ...
                    return [
                        ...
                        sekirei_arm_controller_spawner,
                    ]
                time.sleep(0.5)
            # Timeout -> still start spawners to avoid deadlock in constrained envs
            print(
                "[demo.launch] Warning: controller_manager services not visible; starting spawners anyway"
            )
        ...
        ros2_nodes.append(
            RegisterEventHandler(
                OnProcessStart(
                    ...
                    on_start=[OpaqueFunction(function=_start_spawners_when_ready)],
                )
            )
        )
    except PackageNotFoundError:
        print(
            "[launch warning] controller_manager package not found; skipping ros2_control_node and spawners. Falling back to MoveIt simple controller manager (fake controllers). Install ros2_control/ros2_controllers to enable hardware controllers."
        )
    ...
    move_group_node = Node(
        ...
        arguments=[
            ...
            "INFO",
            "--log-level",
            "moveit.trajectory_execution:=DEBUG",
        ],
    )
    ...
    robot_state_publisher = Node(
        ...
        parameters=[
            ...
            },  # Don't republish, ros2_control_node already does
        ],
    )
    ...
    def _start_servo_when_ready(context, *a, **k):
        ...
        print(
            "[demo.launch] Warning: move_group services not visible after timeout; starting servo anyway"
        )
        return [servo_node]
