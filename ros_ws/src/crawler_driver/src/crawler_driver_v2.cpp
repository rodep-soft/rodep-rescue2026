...
public:
  CrawlerDriverV2()
    ...
    velocity_sub_ = create_subscription<custom_interfaces::msg::CrawlerVelocity>(
        "/crawler_driver", 10, std::bind(&CrawlerDriverV2::velocityCallback, this, _1));

    estop_sub_ = create_subscription<std_msgs::msg::Bool>(
        "/emergency_stop", 10, std::bind(&CrawlerDriverV2::estopCallback, this, _1));
...
private:
  ...
  void initParams() {
    ...
    RCLCPP_INFO(get_logger(), "  M1 PID: P=%.3f I=%.3f D=%.3f QPPS=%d", m1_pid_.p, m1_pid_.i,
                m1_pid_.d, m1_pid_.qpps);
    RCLCPP_INFO(get_logger(), "  M2 PID: P=%.3f I=%.3f D=%.3f QPPS=%d", m2_pid_.p, m2_pid_.i,
                m2_pid_.d, m2_pid_.qpps);
  }
  ...
  void velocityCallback(const custom_interfaces::msg::CrawlerVelocity::SharedPtr msg) {
    // E-stop チェック
    if (estop_active_) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000, "⚠️ E-stop有効: モーターコマンド無視");
      return;
    }
    ...
    roboclaw_->setVelocity(Motor::M1, m1_counts, [this](bool success) {
      if (!success) {
        RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), 1000, "M1 速度設定失敗");
      }
    });
    ...
    roboclaw_->setVelocity(Motor::M2, m2_counts, [this](bool success) {
      if (!success) {
        RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), 1000, "M2 速度設定失敗");
      }
    });
  }
