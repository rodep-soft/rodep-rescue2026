...
struct mem_location {
    ...
    inline const char* get_trimmed_filename() {
        ...
        while ((s2 = strstr(s1, "gcc/"))) s1 = s2 + 4;
...
struct mem_usage {
    ...
    inline void register_overhead(size_t size) {
        ...
        if (m_peak < m_allocated) m_peak = m_allocated;
    }
    ...
    inline void dump(mem_location* loc, mem_usage& total) const {
        ...
        fprintf(stderr,
                ...
                get_percent(m_allocated, total.m_allocated), (uint64_t)m_peak,
                (uint64_t)m_times, get_percent(m_times, total.m_times),
                loc->m_ggc ? "ggc" : "heap");
...
class mem_alloc_description {
   public:
    ...
    /* Register memory allocation descriptor for container PTR.  ORIGIN
       identifies type of container and GGC identifes if the allocation is
       handled in GGC memory.  Each location is identified by file NAME, LINE in
    ...
    /* For containers (and GGC) where we want to track every instance object,
       we register allocation of SIZE bytes, identified by PTR pointer,
       belonging to USAGE descriptor.  */
...
template <class T>
inline bool mem_alloc_description<T>::contains_descriptor_for_instance(
    const void* ptr) {
...
template <class T>
inline T* mem_alloc_description<T>::get_descriptor_for_instance(
    const void* ptr) {
...
inline T* mem_alloc_description<T>::register_descriptor(
    const void* ptr, mem_location* location) {
    T* usage = NULL;
...
template <class T>
inline T* mem_alloc_description<T>::register_instance_overhead(
...
template <class T>
inline void mem_alloc_description<T>::release_instance_overhead(
    void* ptr, size_t size, bool remove_from_map) {
    mem_usage_pair<T>* slot = m_reverse_map->get(ptr);
    ...
    if (remove_from_map) m_reverse_map->remove(ptr);
}
...
template <class T>
inline mem_alloc_description<T>::mem_alloc_description() {
...
inline T mem_alloc_description<T>::get_sum(mem_alloc_origin origin) {
    ...
    for (unsigned i = 0; i < length; i++) sum = sum + *list[i].second;
