...
inline void dump_possible_polymorphic_call_targets(FILE* f,
    ...
    dump_possible_polymorphic_call_targets(
        f, e->indirect_info->otr_type, e->indirect_info->otr_token, context);
}
...
inline bool type_with_linkage_p(const_tree t) {
    ...
    if (!TYPE_CONTEXT(t)) return false;
...
inline bool type_in_anonymous_namespace_p(const_tree t) {
    ...
    if (!TREE_PUBLIC(TYPE_STUB_DECL(t))) {
       ...
       verify that this match with type_in_anonymous_namespace_p.  */
        gcc_checking_assert(
            !in_lto_p || !DECL_ASSEMBLER_NAME_SET_P(TYPE_NAME(t)) ||
            !strcmp("<anon>",
...
inline bool odr_type_p(const_tree t) {
    ...
    if (!type_with_linkage_p(t)) return false;

    /* To support -fno-lto-odr-type-merging consider types with vtables ODR.  */
    if (type_in_anonymous_namespace_p(t)) return true;
