...
class __atomic_futex_unsigned : __atomic_futex_unsigned_base {
   ...
   private:
    ...
    unsigned _M_load_and_test_until(unsigned __assumed, unsigned __operand,
                                    ...
                                    chrono::nanoseconds __ns) {
        for (;;) {
            // Don't bother checking the value again because we expect the
            // caller to have done it recently. memory_order_relaxed is
            // sufficient because we can rely on just the modification order
            // (store_notify uses an atomic RMW operation too), and the futex
            // syscalls synchronize between themselves.
