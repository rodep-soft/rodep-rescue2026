...
struct _Bucket_index_helper<_UnorderedCont, std::pair<const _Key, _Mapped>,
                            false> {
...
class _Unordered_profile {
   ...
   protected:
    _Unordered_profile() noexcept { _M_profile_construct(); }
    ...
    ~_Unordered_profile() { _M_profile_destruct(); }
    ...
    void _M_profile_destruct() noexcept {
        auto& __uc = _M_conjure();
        __profcxx_hashtable_size_destruct(_M_size_info, __uc.bucket_count(),
                                          __uc.size());
        _M_size_info = 0;

        if (!_M_hashfunc_info) return;
    ...
    void _M_profile_resize(std::size_t __old_size) {
        ...
        if (__old_size != __new_size)
            __profcxx_hashtable_size_resize(_M_size_info, __old_size,
                                            __new_size);
    }
...
template <typename _UnorderedCont, bool _Unique_keys>
void _Unordered_profile<_UnorderedCont, _Unique_keys>::_M_profile_destruct(
    std::true_type) {
    auto& __uc = _M_conjure();
    ...
    while (__it != __uc.end()) {
        ...
        for (++__it, ++__lit; __lit != __lend; ++__it, ++__lit) ++__chain;
