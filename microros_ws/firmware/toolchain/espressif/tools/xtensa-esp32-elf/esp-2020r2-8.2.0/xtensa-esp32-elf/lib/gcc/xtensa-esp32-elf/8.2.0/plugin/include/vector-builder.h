...
template <typename T, typename Derived>
void vector_builder<T, Derived>::new_vector(poly_uint64 full_nelts,
...
bool vector_builder<T, Derived>::operator==(const Derived& other) const {
    ...
    for (unsigned int i = 0; i < nelts; ++i)
        if (!derived()->equal_p((*this)[i], other[i])) return false;
...
template <typename T, typename Derived>
T vector_builder<T, Derived>::elt(unsigned int i) const {
    ...
    if (i < this->length()) return (*this)[i];
    ...
    if (m_nelts_per_pattern <= 2) return final;
...
template <typename T, typename Derived>
void vector_builder<T, Derived>::reshape(unsigned int npatterns,
                                         unsigned int nelts_per_pattern) {
    ...
    for (unsigned int i = new_encoded_nelts; i < old_encoded_nelts; ++i) {
        ...
        next += 1;
        if (next == new_encoded_nelts) next -= npatterns;
    }
    m_npatterns = npatterns;
    m_nelts_per_pattern = nelts_per_pattern;
}
...
template <typename T, typename Derived>
bool vector_builder<T, Derived>::repeating_sequence_p(unsigned int start,
                                                      unsigned int end,
                                                      unsigned int step) {
    for (unsigned int i = start; i < end - step; ++i)
        if (!derived()->equal_p((*this)[i], (*this)[i + step])) return false;
    return true;
}
...
template <typename T, typename Derived>
bool vector_builder<T, Derived>::stepped_sequence_p(unsigned int start,
                                                    unsigned int end,
                                                    unsigned int step) {
    if (!derived()->allow_steps_p()) return false;
    ...
    for (unsigned int i = start + step * 2; i < end; ++i) {
        ...
        if (!derived()->can_elide_p(elt3)) return false;
    }
    return true;
}
...
bool vector_builder<T, Derived>::try_npatterns(unsigned int npatterns) {
    if (m_nelts_per_pattern == 1) {
       ...
       elements are still encoded explicitly.  */
        if (!encoded_full_vector_p()) return false;
    }
...
template <typename T, typename Derived>
void vector_builder<T, Derived>::finalize() {
    ...
    if (pow2p_hwi(m_npatterns)) {
       ...
       duplicates in the loop above.  */
        ...
        for (unsigned int i = 1; i <= m_npatterns / 2; ++i)
            if (m_npatterns % i == 0 && try_npatterns(i)) break;
}
