   ...
   public:
    ...
    void __write(FILE* __f) const {
        std::fprintf(__f, "%Zu %Zu %Zu %Zu %Zu %Zu %Zu %Zu %Zu %Zu\n", _M_init,
                     _M_count, _M_cost, _M_resize, _M_min, _M_max, _M_total,
    ...
    float __resize_cost(std::size_t __from, std::size_t) { return __from; }
...
class __trace_container_size
   ...
   public:
    ...
    __container_size_info* __insert(__stack_t __stack, std::size_t __num) {
        ...
        if (__ret) __ret->__init(__num);
        return __ret;
    }

    // Call at destruction/clean to set container final size.
    void __destruct(__container_size_info* __obj_info, std::size_t __num,
                    std::size_t __inum) {
