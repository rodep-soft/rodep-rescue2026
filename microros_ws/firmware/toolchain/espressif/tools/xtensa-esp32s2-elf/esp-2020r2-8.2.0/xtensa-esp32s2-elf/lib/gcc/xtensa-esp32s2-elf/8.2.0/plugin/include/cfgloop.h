...
static inline void checking_verify_loop_structure(void) {
    ...
    if (flag_checking) verify_loop_structure();
}
...
inline loop_iterator::loop_iterator(function* fn, loop_p* loop,
                                    unsigned flags) {
    struct loop* aloop;
    unsigned i;
    int mn;
    ...
    if (flags & LI_ONLY_INNERMOST) {
        ...
        while (1) {
            if (aloop->num >= mn) this->to_visit.quick_push(aloop->num);
...
inline loop_iterator::~loop_iterator() { this->to_visit.release(); }
