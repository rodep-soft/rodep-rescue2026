...
inline void _Rope_RopeRep<_CharT, _Alloc>::_S_free_string(_CharT* __s,
                                                          size_t __n,
                                                          allocator_type& __a) {
    if (!_S_is_basic_char_type((_CharT*)0)) _Destroy(__s, __s + __n, __a);
...
typename rope<_CharT, _Alloc>::_RopeRep* rope<_CharT, _Alloc>::_S_concat(
    ...
    if (__detail::_S_leaf == __right->_M_tag) {
        if (__detail::_S_leaf == __left->_M_tag) {
            ...
            if (__leftright->_M_size + __right->_M_size <=
                size_t(_S_copy_max)) {
...
template <class _CharT, class _Alloc>
_CharT* rope<_CharT, _Alloc>::_S_flatten(_RopeRep* __r, size_t __start,
                                         size_t __len, _CharT* __buffer) {
...
int rope<_CharT, _Alloc>::_S_compare(const _RopeRep* __left,
    ...
    if (__detail::_S_leaf == __left->_M_tag) {
        ...
        if (__detail::_S_leaf == __right->_M_tag) {
            ...
            const_iterator __rstart(__right, 0);
            const_iterator __rend(__right, __right_len);
            return lexicographical_compare_3way(__lstart, __lend, __rstart,
                                                __rend);
        }
    }
}
...
template <class _CharT, class _Alloc>
rope<_CharT, _Alloc>::rope(size_t __n, _CharT __c, const allocator_type& __a)
    : _Base(__a) {
    ...
    if (__exponent != 0) {
        ...
        __try {
            __base_leaf = _S_new_RopeLeaf(
                __base_buffer, __exponentiate_threshold, _M_get_allocator());
        }
        __catch(...) {
