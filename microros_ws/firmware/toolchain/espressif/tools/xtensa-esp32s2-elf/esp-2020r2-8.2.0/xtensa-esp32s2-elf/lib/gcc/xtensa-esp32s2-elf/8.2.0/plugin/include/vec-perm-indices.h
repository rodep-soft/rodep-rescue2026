...
inline vec_perm_indices::element_type vec_perm_indices::clamp(
    element_type elt) const {
    element_type limit = input_nelts(), elem_within_input;
    HOST_WIDE_INT input;
    if (!can_div_trunc_p(elt, limit, &input, &elem_within_input)) return elt;

    /* Treat negative elements as counting from the end.  This only matters
       if the vector size is not a power of 2.  */
    if (known_lt(elem_within_input, 0)) return elem_within_input + limit;
...
inline vec_perm_indices::element_type vec_perm_indices::operator[](
