...
/* spellcheck.c  */
extern edit_distance_t levenshtein_distance(const char* s, int len_s,
                                            const char* t, int len_t);

extern edit_distance_t levenshtein_distance(const char* s, const char* t);
...
class best_match {
   public:
    ...
    void consider(candidate_t candidate) {
        ...
        if (min_candidate_distance >= m_best_distance) return;
        ...
        if (min_candidate_distance > cutoff) return;

        /* Otherwise, compute the distance and see if the candidate
           has beaten the previous best value.  */
        edit_distance_t dist = levenshtein_distance(
            m_goal, m_goal_len, candidate_traits::get_string(candidate),
            candidate_len);
    ...
    candidate_t get_best_meaningful_candidate() const {
        ...
        if (m_best_candidate) {
            ...
            if (m_best_distance > cutoff) return NULL;
        }
        ...
        if (m_best_distance == 0) return NULL;
