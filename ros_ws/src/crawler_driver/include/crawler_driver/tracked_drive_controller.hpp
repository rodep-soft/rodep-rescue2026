...
struct TrackedRobotParameters {
  ...
  double meters_per_count() const { return (2.0 * M_PI * track_radius) / total_counts_per_rev(); }

  double counts_per_meter() const { return total_counts_per_rev() / (2.0 * M_PI * track_radius); }
};
...
public:
  explicit TrackedDriveController(const TrackedRobotParameters& params) : params_(params) {}
   ...
   */
  std::pair<double, double> twistToTrackVelocities(double linear_x, double angular_z) const {
    // 基本の差動駆動運動学
    ...
    if (std::abs(angular_z) > 0.1) {
      // 旋回時は滑りが大きい
      const double compensation = params_.turn_slip_ratio * params_.slip_compensation;
      v_left *= compensation;
      v_right *= compensation;
    } else if (std::abs(linear_x) > 0.01) {
      // 直進時の補正（通常は小さい）
      const double compensation = params_.linear_slip_ratio * params_.slip_compensation;
      v_left *= compensation;
      v_right *= compensation;
    }
...
public:
   ...
   */
  static double calculateTurnSlipRatio(double commanded_angle, double actual_angle) {
   ...
   * @return 推定滑り係数
   */
  static double calculateLinearSlipRatio(double commanded_distance, double actual_distance) {
...
public:
  ...
  void update(int32_t left_counts, int32_t right_counts, double dt) {
    ...
    const double corrected_right = right_distance / params_.slip_compensation;

    // 移動距離と回転角度
    const double distance = (corrected_left + corrected_right) / 2.0;
    const double delta_theta = (corrected_right - corrected_left) / params_.track_base;
