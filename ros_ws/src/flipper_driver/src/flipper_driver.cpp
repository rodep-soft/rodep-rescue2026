...
private:
  ...
  void setupDynamixel(uint8_t dxl_id) {
    ...
    if (dxl_comm_result != COMM_SUCCESS) {
      ...
      RCLCPP_INFO(this->get_logger(), "Succeeded to set Velocity Control Mode.");
    }
  ...
  void sendGoalVelocity(uint8_t id, int velocity) {
    dxl_comm_result =
        packetHandler->write4ByteTxRx(portHandler, id, ADDR_GOAL_VELOCITY, velocity, &dxl_error);
    ...
    if (dxl_comm_result != COMM_SUCCESS) {
      ...
      RCLCPP_INFO(this->get_logger(), "Set [ID: %d Goal Velocity: %d]", id, velocity);
    }
  }
};
