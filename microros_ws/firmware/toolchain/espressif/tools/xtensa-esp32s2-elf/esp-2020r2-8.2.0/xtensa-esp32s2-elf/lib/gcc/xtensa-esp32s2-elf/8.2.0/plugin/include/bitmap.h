...
struct bitmap_usage : public mem_usage {
    ...
    inline void dump(mem_location* loc, mem_usage& total) const {
        ...
        fprintf(stderr,
                ...
                m_nsearches, m_search_iter, loc->m_ggc ? "ggc" : "heap");
...
static inline void bitmap_initialize(
    bitmap head, bitmap_obstack* obstack CXX_MEM_STAT_INFO) {
    head->first = head->current = NULL;
    head->obstack = obstack;
    if (GATHER_STATISTICS) bitmap_register(head PASS_MEM_STAT);
}
...
static inline void bmp_iter_and_init(bitmap_iterator* bi, const_bitmap map1,
    ...
    while (1) {
        ...
        if (bi->elt2->indx >= bi->elt1->indx) break;
