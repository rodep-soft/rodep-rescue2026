...
inline const line_map_ordinary* linemap_check_ordinary(
...
inline source_location MAP_START_LOCATION(const line_map* map) {
    return map->start_location;
}

/* Get the starting line number of ordinary map MAP.  */

inline linenum_type ORDINARY_MAP_STARTING_LINE_NUMBER(
...
inline int ORDINARY_MAP_INCLUDER_FILE_INDEX(const line_map_ordinary* ord_map) {
...
inline unsigned int MACRO_MAP_NUM_MACRO_TOKENS(
...
inline source_location* MACRO_MAP_LOCATIONS(const line_map_macro* macro_map) {
    return macro_map->macro_locations;
}

/* Get the location of the expansion point of the macro map MAP.  */

inline source_location MACRO_MAP_EXPANSION_POINT_LOCATION(
    const line_map_macro* macro_map) {
...
/* Returns a pointer to the last ordinary map allocated the line table
   SET.  */
inline line_map_ordinary* LINEMAPS_LAST_ALLOCATED_ORDINARY_MAP(
    const line_maps* set) {
...
inline source_location LINEMAPS_MACRO_LOWEST_LOCATION(const line_maps* set) {
...
bool pure_location_p(line_maps* set, source_location loc);

/* Given location LOC within SET, strip away any packed range information
   or ad-hoc information.  */

extern source_location get_pure_location(line_maps* set, source_location loc);

/* Combine LOC and BLOCK, giving a combined adhoc location.  */

inline source_location COMBINE_LOCATION_DATA(struct line_maps* set,
                                             source_location loc,
                                             source_range src_range,
                                             void* block) {
...
extern source_location linemap_line_start(struct line_maps* set,
                                          linenum_type to_line,
                                          unsigned int max_column_hint);
...
int linemap_location_in_system_header_p(struct line_maps*, source_location);
...
/* Return the location of the last source line within an ordinary
   map.  */
inline source_location LAST_SOURCE_LINE_LOCATION(const line_map_ordinary* map) {
    return (((map[1].start_location - 1 - map->start_location) &
...
/* Returns the map a given map was included from, or NULL if the map
   ...
   another one.  */
inline line_map_ordinary* INCLUDED_FROM(struct line_maps* set,
                                        const line_map_ordinary* ord_map) {
...
/* Encode and return a source_location from a column number. The
   ...
   encoded from.  */
extern source_location linemap_position_for_column(struct line_maps*,
                                                   unsigned int);

/* Encode and return a source location from a given line and
   column.  */
source_location linemap_position_for_line_and_column(line_maps* set,
...
/* Encode and return a source_location starting from location LOC and
   ...
   virtual locations.  */
source_location linemap_position_for_loc_and_offset(struct line_maps* set,
                                                    source_location loc,
                                                    unsigned int offset);
...
/* Return a positive value if PRE denotes the location of a token that
   ...
   otherwise.  */
int linemap_compare_locations(struct line_maps* set, source_location pre,
...
class rich_location {
   public:
    ...
    void add_range(source_location loc, bool show_caret_p);

    void set_range(line_maps* set, unsigned int idx, source_location loc,
                   bool show_caret_p);
    ...
    void add_fixit_insert_before(source_location where,
    ...
    void add_fixit_insert_after(source_location where, const char* new_content);
    ...
    /* Suggest removing the content covered between the start and finish
       of WHERE.  */
    void add_fixit_remove(source_location where);
    ...
    void add_fixit_replace(source_location where, const char* new_content);
   ...
   private:
    ...
    void maybe_add_fixit(source_location start, source_location next_loc,
                         const char* new_content);
...
class fixit_hint {
   public:
    fixit_hint(source_location start, source_location next_loc,
               const char* new_content);
    ...
    bool maybe_append(source_location start, source_location next_loc,
                      const char* new_content);
...
source_location linemap_resolve_location(struct line_maps*, source_location loc,
...
source_location linemap_unwind_toward_expansion(
    struct line_maps*, source_location loc, const struct line_map** loc_map);
...
/* If LOC is the virtual location of a token coming from the expansion
   ...
   *MAP is set to the map of the returned location if the later is
   different from LOC.  */
source_location linemap_unwind_to_first_non_reserved_loc(
    struct line_maps*, source_location loc, const struct line_map** map);
...
/* The rich_location class requires a way to expand source_location instances.
   ...
   symbol.  */
extern expanded_location linemap_client_expand_location_to_spelling_point(
    source_location, enum location_aspect);
