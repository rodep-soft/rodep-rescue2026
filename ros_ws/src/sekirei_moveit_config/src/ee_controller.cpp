...
uint8_t ids[] = {27,28}; // Dynamixel motors of the gripper
uint8_t dxl_error = 0;
...
public:
  DynamixelController() : Node("dynamixel_control_node") {
    ...
    set_velocity_subscriber_ = this->create_subscription<sensor_msgs::msg::Joy>(
        "joy", qos_profile, [this](const sensor_msgs::msg::Joy::SharedPtr msg) {
          if (msg->buttons.empty())
            return;
      

          // Send goal position to each Dynamixel
          if(msg->buttons[1]&& msg->buttons[0]){ // O and X button 同時押し
            readPresentPosition(27, &current_position[0]);
            readPresentPosition(28, &current_position[1]);
            sendGoalPosition(27, current_position[0]); //保持
            sendGoalPosition(28, current_position[1]); //保持
          }else if (msg->buttons[0]) {// X buttonのみ
            sendGoalPosition(27, 2856); //正面から見て右側が閉じる位置
            sendGoalPosition(28, 2134); //正面から見て左側が閉じる位置
          } else if (msg->buttons[1]) {// O buttonのみ
            sendGoalPosition(27, 3549);//正面から見て右側が開く位置
            sendGoalPosition(28, 1376); //正面から見て左側が開く位置
          }
...
private:
  ...
  rclcpp::Subscription<sensor_msgs::msg::Joy>::SharedPtr set_velocity_subscriber_;
  ...
  void setupDynamixel(uint8_t dxl_id) {
    ...
    // Enable Torque
    dxl_comm_result =
        packetHandler->write1ByteTxRx(portHandler, dxl_id, ADDR_TORQUE_ENABLE, 1, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS) {
  ...
  void readPresentPosition(uint8_t id, int* position) {
    dxl_comm_result =
    ...
    if (dxl_comm_result != COMM_SUCCESS) {
      ...
      RCLCPP_INFO(this->get_logger(), "Read [ID: %d Present Position: %d]", id, *position);
    }
  }

  void sendGoalPosition(uint8_t id, int position) {
    dxl_comm_result =
        packetHandler->write4ByteTxRx(portHandler, id, ADDR_GOAL_POSITION, position, &dxl_error);
