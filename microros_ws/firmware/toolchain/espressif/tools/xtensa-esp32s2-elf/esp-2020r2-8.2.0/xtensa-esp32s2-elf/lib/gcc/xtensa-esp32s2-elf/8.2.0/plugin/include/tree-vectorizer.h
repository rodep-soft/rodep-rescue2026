...
typedef hash_map<tree_operand_hash, innermost_loop_behavior*>
    vec_base_alignments;
...
typedef struct _loop_vec_info : public vec_info {
    ...
    /* Statements in the loop that have data references that are candidates for
       a runtime (loop versioning) misalignment check.  */
...
static inline bool nested_in_vect_loop_p(struct loop* loop, gimple* stmt) {
    return (loop->inner && (loop->inner == (gimple_bb(stmt))->loop_father));
}
...
#define STMT_VINFO_GROUPED_ACCESS(S) \
    ((S)->first_element != NULL && (S)->data_ref_info)
...
static inline stmt_vec_info vinfo_for_stmt(gimple* stmt) {
...
static inline void set_vinfo_for_stmt(gimple* stmt, stmt_vec_info info) {
...
static inline gimple* get_earlier_stmt(gimple* stmt1, gimple* stmt2) {
...
static inline gimple* get_later_stmt(gimple* stmt1, gimple* stmt2) {
    unsigned int uid1, uid2;
...
static inline bool is_pattern_stmt_p(stmt_vec_info stmt_info) {
    ...
    if (related_stmt && (related_stmt_info = vinfo_for_stmt(related_stmt)) &&
...
static inline bool is_loop_header_bb_p(basic_block bb) {
    if (bb == (bb->loop_father)->header) return true;
...
static inline int builtin_vectorization_cost(
    enum vect_cost_for_stmt type_of_cost, tree vectype, int misalign) {
...
static inline unsigned add_stmt_cost(void* data, int count,
                                     enum vect_cost_for_stmt kind,
                                     stmt_vec_info stmt_info, int misalign,
                                     enum vect_cost_model_location where) {
    return targetm.vectorize.add_stmt_cost(data, count, kind, stmt_info,
                                           misalign, where);
}

/* Alias targetm.vectorize.finish_cost.  */

static inline void finish_cost(void* data, unsigned* prologue_cost,
...
/*-----------------------------------------------------------------*/
inline void set_dr_misalignment(struct data_reference* dr, int val) {
...
inline int dr_misalignment(struct data_reference* dr) {
...
static inline bool aligned_access_p(struct data_reference* data_ref_info) {
...
static inline bool known_alignment_for_access_p(
    struct data_reference* data_ref_info) {
...
static inline unsigned int vect_known_alignment_in_bytes(
    struct data_reference* dr) {
    if (DR_MISALIGNMENT(dr) == DR_MISALIGNMENT_UNKNOWN)
        return TYPE_ALIGN_UNIT(TREE_TYPE(DR_REF(dr)));
    if (DR_MISALIGNMENT(dr) == 0) return DR_TARGET_ALIGNMENT(dr);
...
static inline innermost_loop_behavior* vect_dr_behavior(data_reference* dr) {
...
static inline bool vect_use_loop_mask_for_alignment_p(
...
static inline unsigned int vect_get_num_vectors(poly_uint64 nunits,
                                                tree vectype) {
...
static inline unsigned int vect_get_num_copies(loop_vec_info loop_vinfo,
                                               tree vectype) {
...
static inline void vect_update_max_nunits(poly_uint64* max_nunits,
                                          tree vectype) {
...
static inline unsigned HOST_WIDE_INT vect_max_vf(loop_vec_info loop_vinfo) {
    ...
    if (LOOP_VINFO_VECT_FACTOR(loop_vinfo).is_constant(&vf)) return vf;
...
inline unsigned int vect_get_scalar_dr_size(struct data_reference* dr) {
...
extern void vect_set_loop_condition(struct loop*, loop_vec_info, tree, tree,
                                    tree, bool);
extern bool slpeel_can_duplicate_loop_p(const struct loop*, const_edge);
struct loop* slpeel_tree_duplicate_loop_to_edge_cfg(struct loop*, struct loop*,
                                                    edge);
extern void vect_loop_versioning(loop_vec_info, unsigned int, bool,
                                 poly_uint64);
extern struct loop* vect_do_peeling(loop_vec_info, tree, tree, tree*, tree*,
                                    tree*, int, bool, bool);
...
extern bool vect_is_simple_use(tree, vec_info*, gimple**, enum vect_def_type*);
extern bool vect_is_simple_use(tree, vec_info*, gimple**, enum vect_def_type*,
                               tree*);
...
extern bool supportable_narrowing_operation(enum tree_code, tree, tree,
                                            enum tree_code*, int*, vec<tree>*);
extern stmt_vec_info new_stmt_vec_info(gimple* stmt, vec_info*);
extern void free_stmt_vec_info(gimple* stmt);
extern void vect_model_simple_cost(stmt_vec_info, int, enum vect_def_type*, int,
...
extern void vect_get_vec_defs(tree, tree, gimple*, vec<tree>*, vec<tree>*,
                              slp_tree);
extern void vect_get_vec_defs_for_stmt_copy(enum vect_def_type*, vec<tree>*,
                                            vec<tree>*);
extern tree vect_init_vector(gimple*, tree, tree, gimple_stmt_iterator*);
extern tree vect_get_vec_def_for_stmt_copy(enum vect_def_type, tree);
extern bool vect_transform_stmt(gimple*, gimple_stmt_iterator*, bool*, slp_tree,
                                slp_instance);
extern void vect_remove_stores(gimple*);
extern bool vect_analyze_stmt(gimple*, bool*, slp_tree, slp_instance);
extern bool vectorizable_condition(gimple*, gimple_stmt_iterator*, gimple**,
                                   tree, int, slp_tree);
extern void vect_get_load_cost(struct data_reference*, int, bool, unsigned int*,
...
extern tree vect_create_data_ref_ptr(gimple*, tree, struct loop*, tree, tree*,
                                     gimple_stmt_iterator*, gimple**, bool,
                                     bool*, tree = NULL_TREE, tree = NULL_TREE);
...
extern tree vect_create_addr_base_for_vector_ref(gimple*, gimple_seq*, tree,
                                                 tree = NULL_TREE);
...
extern bool can_duplicate_and_interleave_p(unsigned int, machine_mode,
                                           ...
                                           tree* = NULL);
extern void duplicate_and_interleave(gimple_seq*, tree, vec<tree>, unsigned int,
                                     vec<tree>&);
