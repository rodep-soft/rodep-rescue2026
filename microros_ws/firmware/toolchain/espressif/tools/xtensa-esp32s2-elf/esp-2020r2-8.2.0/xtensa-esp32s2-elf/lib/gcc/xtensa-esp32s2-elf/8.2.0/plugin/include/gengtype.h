...
options_p create_nested_ptr_option(options_p, type_p t, const char* from,
                                   const char* to);
...
static inline struct fileloc* type_fileloc(type_p t) {
    if (!t) return NULL;
    if (union_or_struct_p(t)) return &t->u.s.line;
    return NULL;
}
...
#define DBGPRINT_COUNT_TYPE(Msg, Ty)                                       \
    do {                                                                   \
        if (do_debug) dbgprint_count_type_at(__FILE__, __LINE__, Msg, Ty); \
    } while (0)

#define FOR_ALL_INHERITED_FIELDS(TYPE, FIELD_VAR)             \
    for (type_p sub = (TYPE); sub; sub = sub->u.s.base_class) \
        for (FIELD_VAR = sub->u.s.fields; FIELD_VAR;          \
             FIELD_VAR = FIELD_VAR->next)
