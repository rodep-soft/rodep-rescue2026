...
extern bool compute_all_dependences(vec<data_reference_p>, vec<ddr_p>*,
                                    vec<loop_p>, bool);
...
static inline bool same_data_refs_base_objects(data_reference_p a,
                                               data_reference_p b) {
...
static inline bool same_data_refs(data_reference_p a, data_reference_p b) {
    ...
    if (operand_equal_p(DR_REF(a), DR_REF(b), 0)) return true;

    if (!same_data_refs_base_objects(a, b)) return false;
...
inline bool known_dependences_p(vec<ddr_p> dependence_relations) {
    ...
    if (DDR_ARE_DEPENDENT(ddr) == chrec_dont_know) return false;
...
static inline bool adjacent_dr_p(struct data_reference* dr) {
    ...
    if (!DR_STEP(dr) || TREE_CODE(DR_STEP(dr)) != INTEGER_CST) return false;
...
static inline int lambda_vector_gcd(lambda_vector vector, int size) {
    int i;
    int gcd1 = 0;

    if (size > 0) {
        gcd1 = vector[0];
        for (i = 1; i < size; i++) gcd1 = gcd(gcd1, vector[i]);
    }
    return gcd1;
}
...
static inline lambda_matrix lambda_matrix_new(int m, int n,
    ...
    for (i = 0; i < m; i++) mat[i] = XOBNEWVEC(lambda_obstack, int, n);
