...
class cp_expr {
   public:
    ...
    cp_expr(tree value) : m_value(value), m_loc(EXPR_LOCATION(m_value)) {}
...
/* Usage of TREE_LANG_FLAG_?:
   ...
   0: DECL_ERROR_REPORTED (in VAR_DECL).
      DECL_TEMPLATE_PARM_P (in PARM_DECL, CONST_DECL, TYPE_DECL, or
...
inline lang_identifier* identifier_p(tree t) {
    if (TREE_CODE(t) == IDENTIFIER_NODE) return (lang_identifier*)t;
    return NULL;
}
...
inline tree_constraint_info* check_constraint_info(tree t) {
    if (t && TREE_CODE(t) == CONSTRAINT_INFO) return (tree_constraint_info*)t;
    return NULL;
}
...
union GTY(
    ...
    struct tree_deferred_noexcept GTY((tag("TS_CP_DEFERRED_NOEXCEPT")))
        deferred_noexcept;
...
struct processing_template_decl_sentinel {
    ...
    processing_template_decl_sentinel(bool reset = true)
        ...
        if (reset) processing_template_decl = 0;
    }
...
#define LANG_DECL_U2_CHECK(NODE, TF) (&DECL_LANG_SPECIFIC(NODE)->u.min.u2)
...
/* For FUNCTION_DECLs and TEMPLATE_DECLs: nonzero means that this function
   is a constructor.  */
#define DECL_CONSTRUCTOR_P(NODE) IDENTIFIER_CTOR_P(DECL_NAME(NODE))
...
/* Nonzero if NODE (a FUNCTION_DECL or TEMPLATE_DECL)
   is a destructor.  */
#define DECL_DESTRUCTOR_P(NODE) IDENTIFIER_DTOR_P(DECL_NAME(NODE))
...
#define DECL_DISCRIMINATOR_P(NODE) (VAR_P(NODE) && DECL_FUNCTION_SCOPE_P(NODE))
...
#define DECL_SHADOWED_FOR_VAR(NODE)                                         \
    (DECL_HAS_SHADOWED_FOR_VAR_P(NODE) ? decl_shadowed_for_var_lookup(NODE) \
                                       : NULL)
...
#define DECL_CAPTURED_VARIABLE(NODE) (LANG_DECL_U2_CHECK(NODE, 0)->access)
...
#define REF_PARENTHESIZED_P(NODE) \
    TREE_LANG_FLAG_2(             \
        TREE_CHECK3((NODE), COMPONENT_REF, INDIRECT_REF, SCOPE_REF))
...
inline void init_aggr_init_expr_arg_iterator(
    tree exp, aggr_init_expr_arg_iterator* iter) {
    iter->t = exp;
...
inline tree next_aggr_init_expr_arg(aggr_init_expr_arg_iterator* iter) {
    tree result;
    if (iter->i >= iter->n) return NULL_TREE;
...
inline bool more_aggr_init_expr_args_p(
...
#define TYPE_OBJ_P(NODE)                                        \
    (TREE_CODE(NODE) != REFERENCE_TYPE && !VOID_TYPE_P(NODE) && \
...
inline bool variable_template_p(tree t) {
    if (TREE_CODE(t) != TEMPLATE_DECL) return false;
    if (!PRIMARY_TEMPLATE_P(t)) return false;
    if (tree r = DECL_TEMPLATE_RESULT(t)) return VAR_P(r);
    return false;
}
...
extern tree build_special_member_call(tree, tree, vec<tree, va_gc>**, tree, int,
                                      tsubst_flags_t);
extern tree build_new_op(location_t, enum tree_code, int, tree, tree, tree,
                         tree*, tsubst_flags_t);
...
extern tree fn_type_unification(tree, tree, tree, const tree*, unsigned int,
                                tree, unification_kind_t, int, bool, bool);
...
extern tree finish_call_expr(tree, vec<tree, va_gc>**, bool, bool,
                             tsubst_flags_t);
extern tree lookup_and_finish_template_variable(
    tree, tree, tsubst_flags_t = tf_warning_or_error);
...
extern void finish_omp_atomic(enum tree_code, enum tree_code, tree, tree, tree,
                              tree, tree, bool);
...
extern tree cp_build_function_call_vec(tree, vec<tree, va_gc>**,
                                       tsubst_flags_t);
extern tree build_x_binary_op(location_t, enum tree_code, tree, enum tree_code,
                              tree, enum tree_code, tree*, tsubst_flags_t);
...
extern tree cp_build_binary_op(location_t, enum tree_code, tree, tree,
                               tsubst_flags_t);
...
inline tree ovl_first(tree node) {
    while (TREE_CODE(node) == OVERLOAD) node = OVL_FUNCTION(node);
    return node;
}
