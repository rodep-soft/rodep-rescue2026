...
#endif
    for (; n; ++dst, --n) ::new (static_cast<void*>(dst)) T();
}
...
inline void vec_copy_construct(T* dst, const T* src, unsigned n) {
    for (; n; ++dst, ++src, --n) ::new (static_cast<void*>(dst)) T(*src);
}
...
inline bool vec_safe_reserve(vec<T, A, vl_embed>*& v, unsigned nelems,
    ...
    if (extend) A::reserve(v, nelems, exact PASS_MEM_STAT);
    return extend;
}
...
inline void vec_safe_truncate(vec<T, A, vl_embed>* v, unsigned size) {
    if (v) v->truncate(size);
}

/* If SRC is not NULL, return a pointer to a copy of it.  */
template <typename T, typename A>
inline vec<T, A, vl_embed>* vec_safe_copy(
    vec<T, A, vl_embed>* src CXX_MEM_STAT_INFO) {
...
template <typename T, typename A>
inline vec<T, A, vl_embed>* vec<T, A, vl_embed>::copy(
    ALONE_MEM_STAT_DECL) const {
...
inline void vec<T, A, vl_embed>::qsort(int (*cmp)(const void*, const void*)) {
    if (length() > 1) ::qsort(address(), length(), sizeof(T), cmp);
}
...
inline void vec<T, A, vl_embed>::quick_grow_cleared(unsigned len) {
    ...
    quick_grow(len);
    if (growby != 0) vec_default_construct(address() + oldlen, growby);
}
...
void gt_ggc_mx(vec<T, va_gc>* v) {
    ...
    for (unsigned i = 0; i < v->length(); i++) gt_ggc_mx((*v)[i]);
}
...
void gt_pch_nx(vec<T, A, vl_embed>* v) {
    ...
    for (unsigned i = 0; i < v->length(); i++) gt_pch_nx((*v)[i]);
}

template <typename T, typename A>
void gt_pch_nx(vec<T*, A, vl_embed>* v, gt_pointer_operator op, void* cookie) {
    for (unsigned i = 0; i < v->length(); i++) op(&((*v)[i]), cookie);
}
...
struct vec<T, va_heap, vl_ptr> {
   public:
    ...
    T* address(void) { return m_vec ? m_vec->m_vecdata : NULL; }

    const T* address(void) const { return m_vec ? m_vec->m_vecdata : NULL; }
...
inline void vec_check_alloc(vec<T, va_heap>*& vec,
                            unsigned nelems CXX_MEM_STAT_INFO) {
    if (!vec) vec_alloc(vec, nelems PASS_MEM_STAT);
}
...
template <typename T>
inline vec<T, va_heap, vl_ptr> vec<T, va_heap, vl_ptr>::copy(
    ALONE_MEM_STAT_DECL) const {
    vec<T, va_heap, vl_ptr> new_vec = vNULL;
    if (length()) new_vec.m_vec = m_vec->copy();
    return new_vec;
}
...
template <typename T>
inline bool vec<T, va_heap, vl_ptr>::reserve_exact(
    unsigned nelems MEM_STAT_DECL) {
...
inline void vec<T, va_heap, vl_ptr>::create(unsigned nelems MEM_STAT_DECL) {
    m_vec = NULL;
    if (nelems > 0) reserve_exact(nelems PASS_MEM_STAT);
}
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::splice(
    const vec<T, va_heap, vl_ptr>& src) {
    if (src.m_vec) m_vec->splice(*(src.m_vec));
}
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_grow_cleared(
    unsigned len MEM_STAT_DECL) {
    unsigned oldlen = length();
    size_t growby = len - oldlen;
...
inline T* vec<T, va_heap, vl_ptr>::bsearch(const void* key,
                                           int (*cmp)(const void*,
                                                      const void*)) {
    if (m_vec) return m_vec->bsearch(key, cmp);
    return NULL;
}
...
inline void release_vec_vec(vec<vec<T> >& vec) {
    for (unsigned i = 0; i < vec.length(); i++) vec[i].release();
