...
static inline tree redirect_edge_var_map_def(edge_var_map* v) { return v->def; }
...
static inline source_location redirect_edge_var_map_location(edge_var_map* v) {
    return v->locus;
}
...
static inline void checking_verify_ssa(bool check_modified_stmt,
    ...
    if (flag_checking) verify_ssa(check_modified_stmt, check_ssa_operands);
}
