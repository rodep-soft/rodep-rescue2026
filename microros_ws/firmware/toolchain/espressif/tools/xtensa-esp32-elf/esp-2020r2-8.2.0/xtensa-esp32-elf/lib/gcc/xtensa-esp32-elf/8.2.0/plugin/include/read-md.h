...
class md_reader {
   public:
    ...
    bool read_file_fragment(const char* filename, int first_line,
                            int last_line);
    ...
    void record_potential_iterator_use(struct iterator_group* group, rtx x,
   ...
   private:
    ...
    /* A hash table of triples (A, B, C), where each of A, B and C is a
       condition and A is equivalent to "B && C".  This is used to keep track of
       the source of conditions that are made up of separate MD strings (such as
       the split condition of a define_insn_and_split).  */
...
static inline int read_char(void) { return md_reader_ptr->read_char(); }

/* Put back CH, which was the last character read from the MD file.  */

static inline void unread_char(int ch) { md_reader_ptr->unread_char(ch); }
