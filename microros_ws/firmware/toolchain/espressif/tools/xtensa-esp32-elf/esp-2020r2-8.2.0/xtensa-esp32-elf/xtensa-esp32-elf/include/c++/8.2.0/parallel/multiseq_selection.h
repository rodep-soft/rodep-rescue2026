                                    ...
                                    value_type>())  // std::less<_Tp>
{
    ...
    for (_SeqNumber __i = 0; __i < __m; __i++) {
        ...
        _GLIBCXX_PARALLEL_ASSERT(std::distance(__begin_seqs[__i].first,
...
#define __S(__i) (__begin_seqs[__i].first)
    ...
    _SeqNumber __j;
    for (__j = 0;
         __j < __localrank && ((__n + 1) <= __ns[__sample[__j].second]); ++__j)
...
#define __S(__i) (__begin_seqs[__i].first)
    ...
    if (!__maxleftset || __comp(__minright, __maxleft)) {
        ...
        for (_SeqNumber __i = 0; __i < __m; ++__i) {
            _DifferenceType lb =
                std::lower_bound(__S(__i), __S(__i) + __ns[__i], __minright,
                                 __comp) -
                __S(__i);
            __offset += __a[__i] - lb;
        }
