...
extern bool convert_affine_scev(struct loop*, tree, tree*, tree*, gimple*, bool,
                                tree = NULL);
...
static inline bool chrec_zerop(const_tree chrec) {
    if (chrec == NULL_TREE) return false;

    if (TREE_CODE(chrec) == INTEGER_CST) return integer_zerop(chrec);
...
static inline bool no_evolution_in_loop_p(tree chrec, unsigned loop_num,
                                          bool* res) {
    tree scev;
...
static inline tree build_polynomial_chrec(unsigned loop_num, tree left,
                                          tree right) {
    bool val;
    ...
    if (POINTER_TYPE_P(TREE_TYPE(left)))
       ...
       the base of the chrec, and not in the step.  */
        gcc_checking_assert(
            !POINTER_TYPE_P(TREE_TYPE(right)) &&
            types_compatible_p(TREE_TYPE(left), TREE_TYPE(right)));
    }

    if (chrec_zerop(right)) return left;
...
static inline bool evolution_function_is_constant_p(const_tree chrec) {
    ...
    if (CONSTANT_CLASS_P(chrec)) return true;
    return is_gimple_min_invariant(chrec);
}

/* Determine whether CHREC is an affine evolution function in LOOPNUM.  */

static inline bool evolution_function_is_affine_in_loop(const_tree chrec,
                                                        int loopnum) {
    if (chrec == NULL_TREE) return false;
...
static inline tree chrec_type(const_tree chrec) {
    if (automatically_generated_chrec_p(chrec)) return NULL_TREE;

    return TREE_TYPE(chrec);
}

static inline tree chrec_fold_op(enum tree_code code, tree type, tree op0,
                                 tree op1) {
    switch (code) {
        case PLUS_EXPR:
