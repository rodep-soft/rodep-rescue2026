...
#define IRA_BB_NODE_BY_INDEX(index)                                            \
    __extension__(({                                                           \
        ...
        if (_node->children != NULL || _node->loop != NULL ||                  \
            _node->bb == NULL) {                                               \
...
static inline void minmax_set_iter_init(minmax_set_iterator* i,
...
static inline bool minmax_set_iter_cond(minmax_set_iterator* i, int* n) {
    ...
    for (; i->word == 0; i->word = i->vec[i->word_num]) {
        ...
        if (i->bit_num >= i->nel) return false;
    }
...
struct target_ira_int {
    ...
    /* Array of the number of hard registers of given class which are
       ...
       register numbers.  */
    short x_ira_non_ordered_class_hard_regs[N_REG_CLASSES]
    ...
    /* Index [CL][M] contains R if R appears somewhere in a register of the
       form:
...
static inline bool ira_equiv_no_lvalue_p(int regno) {
    if (regno >= ira_reg_equiv_len) return false;
...
static inline void ira_init_register_move_cost_if_necessary(machine_mode mode) {
    if (ira_register_move_cost[mode] == NULL) ira_init_register_move_cost(mode);
}
...
static inline void ira_allocno_iter_init(ira_allocno_iterator* i) { i->n = 0; }

/* Return TRUE if we have more allocnos to visit, in which case *A is
   set to the allocno to be visited.  Otherwise, return FALSE.  */
static inline bool ira_allocno_iter_cond(ira_allocno_iterator* i,
                                         ira_allocno_t* a) {
    int n;
...
/* Initialize the iterator I.  */
static inline void ira_object_iter_init(ira_object_iterator* i) { i->n = 0; }

/* Return TRUE if we have more objects to visit, in which case *OBJ is
   set to the object to be visited.  Otherwise, return FALSE.  */
static inline bool ira_object_iter_cond(ira_object_iterator* i,
                                        ira_object_t* obj) {
    int n;
...
/* Initialize the iterator I.  */
static inline void ira_allocno_object_iter_init(
...
/* Return TRUE if we have more objects to visit in allocno A, in which
   ...
   FALSE.  */
static inline bool ira_allocno_object_iter_cond(ira_allocno_object_iterator* i,
                                                ira_allocno_t a,
                                                ira_object_t* o) {
    int n = i->n++;
...
/* Initialize the iterator I.  */
static inline void ira_pref_iter_init(ira_pref_iterator* i) { i->n = 0; }
...
/* Initialize the iterator I.  */
static inline void ira_copy_iter_init(ira_copy_iterator* i) { i->n = 0; }
...
static inline void ira_object_conflict_iter_init(
    ira_object_conflict_iterator* i, ira_object_t obj) {
...
static inline bool ira_object_conflict_iter_cond(
    ira_object_conflict_iterator* i, ira_object_t* pobj) {
    ira_object_t obj;
    ...
    if (i->conflict_vec_p) {
        ...
        for (; word == 0; word = ((IRA_INT_TYPE*)i->vec)[i->word_num]) {
            ...
            /* If we have reached the end, break.  */
            if (i->word_num * sizeof(IRA_INT_TYPE) >= i->size) return false;

            bit_num = i->word_num * IRA_INT_BITS;
        }

        /* Skip bits that are zero.  */
        for (; (word & 1) == 0; word >>= 1) bit_num++;
...
/* The function returns TRUE if at least one hard register from ones
   ...
   HARD_REGSET.  */
static inline bool ira_hard_reg_set_intersection_p(int hard_regno,
                                                   machine_mode mode,
    ...
    for (i = hard_regno_nregs(hard_regno, mode) - 1; i >= 0; i--)
        if (TEST_HARD_REG_BIT(hard_regset, hard_regno + i)) return true;
    return false;
}
...
static inline int hard_reg_set_size(HARD_REG_SET set) {
    ...
    for (size = i = 0; i < FIRST_PSEUDO_REGISTER; i++)
        if (TEST_HARD_REG_BIT(set, i)) size++;
    return size;
}
...
static inline bool ira_hard_reg_in_set_p(int hard_regno, machine_mode mode,
    ...
    for (i = hard_regno_nregs(hard_regno, mode) - 1; i >= 0; i--)
        if (!TEST_HARD_REG_BIT(hard_regset, hard_regno + i)) return false;
    return true;
}
...
static inline void ira_allocate_and_set_costs(int** vec, reg_class_t aclass,
                                              int val) {
    int i, *reg_costs;
    int len;
    ...
    *vec = reg_costs = ira_allocate_cost_vector(aclass);
    ...
    for (i = 0; i < len; i++) reg_costs[i] = val;
}

/* Allocate cost vector *VEC for hard registers of ACLASS and copy
   values of vector SRC into the vector if it is necessary */
static inline void ira_allocate_and_copy_costs(int** vec, enum reg_class aclass,
                                               int* src) {
    int len;

    if (*vec != NULL || src == NULL) return;
...
/* Allocate cost vector *VEC for hard registers of ACLASS and add
   values of vector SRC into the vector if it is necessary */
static inline void ira_allocate_and_accumulate_costs(int** vec,
    ...
    for (i = 0; i < len; i++) (*vec)[i] += src[i];
}

/* Allocate cost vector *VEC for hard registers of ACLASS and copy
   values of vector SRC into the vector or initialize it by VAL (if
   SRC is null).  */
static inline void ira_allocate_and_set_or_copy_costs(int** vec,
                                                      enum reg_class aclass,
                                                      int val, int* src) {
    int i, *reg_costs;
    int len;
