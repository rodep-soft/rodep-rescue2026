...
struct cpp_callbacks {
    ...
    /* Called to emit a diagnostic.  This callback receives the
       translated message.  */
    bool (*error)(cpp_reader*, int, int, rich_location*, const char*, va_list*)
    ...
    void (*comment)(cpp_reader*, source_location, const unsigned char*, size_t);
...
#define CPP_HASHNODE_VALUE_IDX(HNODE)                               \
    ((HNODE.flags & NODE_MACRO_ARG) ? NTV_ARGUMENT                  \
     : HNODE.type == NT_MACRO                                       \
         ? ((HNODE.flags & NODE_BUILTIN) ? NTV_BUILTIN : NTV_MACRO) \
...
class cpp_string_location_reader {
   public:
    cpp_string_location_reader(source_location src_loc, line_maps* line_table);
...
extern const char* cpp_interpret_string_ranges(
    cpp_reader* pfile, const cpp_string* from, cpp_string_location_reader*,
    size_t count, cpp_substring_ranges* out, enum cpp_ttype type);
...
extern bool cpp_warning_with_line(cpp_reader*, int, source_location, unsigned,
                                  const char* msgid, ...) ATTRIBUTE_PRINTF_5;
...
extern bool cpp_error_at(cpp_reader* pfile, int level, source_location src_loc,
                         const char* msgid, ...) ATTRIBUTE_PRINTF_4;

extern bool cpp_error_at(cpp_reader* pfile, int level, rich_location* richloc,
                         const char* msgid, ...) ATTRIBUTE_PRINTF_4;
