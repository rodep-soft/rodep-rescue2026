...
class hash {
   public:
    ...
    void add_poly_int(const poly_int_pod<N, T>& v) {
        for (unsigned int i = 0; i < N; ++i) add_int(v.coeffs[i]);
    }
    ...
    void add_poly_hwi(const poly_int_pod<N, T>& v) {
        for (unsigned int i = 0; i < N; ++i) add_hwi(v.coeffs[i]);
    }
    ...
    void add_wide_int(const generic_wide_int<T>& x) {
        add_int(x.get_len());
        for (unsigned i = 0; i < x.get_len(); i++) add_hwi(x.elt(i));
    }
    ...
    void merge(hash& other) { merge_hash(other.val); }
