...
struct GTY((tag("GSS_OMP_SINGLE_LAYOUT"))) gomp_teams
    : public gimple_statement_omp_single_layout {
    /* No extra fields; adds invariant:
...
template <>
template <>
inline bool is_a_helper<const gimple_statement_omp_return*>::test(
    const gimple* gs) {
    return gs->code == GIMPLE_OMP_RETURN;
}
...
template <>
template <>
inline bool is_a_helper<const gimple_statement_omp_taskreg*>::test(
    const gimple* gs) {
    return gs->code == GIMPLE_OMP_PARALLEL || gs->code == GIMPLE_OMP_TASK;
}
...
static inline gimple_seq_node gimple_seq_first(gimple_seq s) { return s; }
...
static inline bool gimple_seq_empty_p(gimple_seq s) { return s == NULL; }
...
static inline enum gimple_code gimple_code(const gimple* g) { return g->code; }

/* Return the GSS code used by a GIMPLE code.  */

static inline enum gimple_statement_structure_enum gss_for_code(
...
static inline enum gimple_statement_structure_enum gimple_statement_structure(
    gimple* gs) {
...
static inline basic_block gimple_bb(const gimple* g) { return g->bb; }
...
static inline bool gimple_visited_p(gimple* stmt) { return stmt->visited; }
...
static inline void gimple_set_uid(gimple* g, unsigned uid) { g->uid = uid; }
...
static inline unsigned gimple_uid(const gimple* g) { return g->uid; }
...
template <>
template <>
inline bool is_a_helper<const gimple_statement_with_ops*>::test(
    const gimple* gs) {
...
template <>
template <>
inline bool is_a_helper<const gimple_statement_with_memory_ops*>::test(
    const gimple* gs) {
...
static inline tree gimple_vuse(const gimple* g) {
    ...
    if (!mem_ops_stmt) return NULL_TREE;
...
static inline void gimple_set_modified(gimple* s, bool modifiedp) {
    if (gimple_has_ops(s)) s->modified = (unsigned)modifiedp;
}
...
static inline void gimple_set_has_volatile_ops(gimple* stmt, bool volatilep) {
    if (gimple_has_mem_ops(stmt)) stmt->has_volatile_ops = (unsigned)volatilep;
}
...
static inline bool gimple_omp_atomic_seq_cst_p(const gimple* g) {
    if (gimple_code(g) != GIMPLE_OMP_ATOMIC_LOAD)
        GIMPLE_CHECK(g, GIMPLE_OMP_ATOMIC_STORE);
...
static inline void gimple_omp_atomic_set_seq_cst(gimple* g) {
    if (gimple_code(g) != GIMPLE_OMP_ATOMIC_LOAD)
        GIMPLE_CHECK(g, GIMPLE_OMP_ATOMIC_STORE);
...
static inline unsigned gimple_num_ops(const gimple* gs) { return gs->num_ops; }
...
static inline tree gimple_assign_lhs(const gassign* gs) { return gs->op[0]; }
...
static inline tree* gimple_assign_lhs_ptr(gassign* gs) { return &gs->op[0]; }
...
static inline void gimple_assign_set_lhs(gassign* gs, tree lhs) {
    ...
    if (lhs && TREE_CODE(lhs) == SSA_NAME) SSA_NAME_DEF_STMT(lhs) = gs;
}
...
static inline tree gimple_assign_rhs1(const gassign* gs) { return gs->op[1]; }
...
static inline tree* gimple_assign_rhs1_ptr(gassign* gs) { return &gs->op[1]; }
...
static inline bool gimple_assign_load_p(const gimple* gs) {
    tree rhs;
    if (!gimple_assign_single_p(gs)) return false;
    rhs = gimple_assign_rhs1(gs);
    if (TREE_CODE(rhs) == WITH_SIZE_EXPR) return true;
...
static inline tree gimple_call_lhs(const gcall* gs) { return gs->op[0]; }
...
static inline tree* gimple_call_lhs_ptr(gcall* gs) { return &gs->op[0]; }
...
static inline bool gimple_call_with_bounds_p(const gcall* gs) {
    return (gs->subcode & GF_CALL_WITH_BOUNDS) != 0;
}

static inline bool gimple_call_with_bounds_p(const gimple* gs) {
    const gcall* gc = GIMPLE_CHECK2<const gcall*>(gs);
...
static inline void gimple_call_set_with_bounds(gcall* gs, bool with_bounds) {
    if (with_bounds)
...
static inline void gimple_call_set_with_bounds(gimple* gs, bool with_bounds) {
    gcall* gc = GIMPLE_CHECK2<gcall*>(gs);
...
static inline void gimple_call_set_ctrl_altering(gcall* s,
...
static inline tree gimple_call_fntype(const gcall* gs) {
    if (gimple_call_internal_p(gs)) return NULL_TREE;
    return gs->u.fntype;
}
...
static inline tree gimple_call_fn(const gcall* gs) { return gs->op[1]; }
...
static inline tree* gimple_call_fn_ptr(gcall* gs) { return &gs->op[1]; }
...
static inline tree gimple_call_return_type(const gcall* gs) {
    ...
    if (type == NULL_TREE) return TREE_TYPE(gimple_call_lhs(gs));
...
static inline tree gimple_call_chain(const gcall* gs) { return gs->op[2]; }
...
static inline void gimple_call_set_by_descriptor(gcall* s,
...
static inline bool gimple_has_lhs(gimple* stmt) {
    if (is_gimple_assign(stmt)) return true;
...
static inline tree gimple_cond_lhs(const gcond* gs) { return gs->op[0]; }
...
static inline tree* gimple_cond_lhs_ptr(gcond* gs) { return &gs->op[0]; }

/* Set LHS to be the LHS operand of the predicate computed by
   conditional statement GS.  */

static inline void gimple_cond_set_lhs(gcond* gs, tree lhs) { gs->op[0] = lhs; }

/* Return the RHS operand of the predicate computed by conditional GS.  */

static inline tree gimple_cond_rhs(const gcond* gs) { return gs->op[1]; }
...
static inline tree* gimple_cond_rhs_ptr(gcond* gs) { return &gs->op[1]; }

/* Set RHS to be the RHS operand of the predicate computed by
   conditional statement GS.  */

static inline void gimple_cond_set_rhs(gcond* gs, tree rhs) { gs->op[1] = rhs; }

/* Return the label used by conditional statement GS when its
   predicate evaluates to true.  */

static inline tree gimple_cond_true_label(const gcond* gs) { return gs->op[2]; }
...
static inline bool gimple_cond_true_p(const gcond* gs) {
    ...
    if (lhs != boolean_true_node && lhs != boolean_false_node) return false;

    if (rhs != boolean_true_node && rhs != boolean_false_node) return false;

    if (code == NE_EXPR && lhs != rhs) return true;

    if (code == EQ_EXPR && lhs == rhs) return true;
...
static inline tree gimple_label_label(const glabel* gs) { return gs->op[0]; }
...
static inline void gimple_try_set_kind(gtry* gs, enum gimple_try_flags kind) {
    ...
    if (gimple_try_kind(gs) != kind) gs->subcode = (unsigned int)kind;
}
...
static inline tree gimple_phi_result(const gphi* gs) { return gs->result; }
...
static inline tree* gimple_phi_result_ptr(gphi* gs) { return &gs->result; }
...
static inline source_location gimple_phi_arg_location(gphi* phi, size_t i) {
    return gimple_phi_arg(phi, i)->locus;
}

/* Return the source location of the argument on edge E of phi node PHI.  */

static inline source_location gimple_phi_arg_location_from_edge(gphi* phi,
                                                                edge e) {
...
static inline int gimple_eh_dispatch_region(
...
static inline tree gimple_switch_index(const gswitch* gs) { return gs->op[0]; }

/* Return a pointer to the index variable for the switch statement GS.  */

static inline tree* gimple_switch_index_ptr(gswitch* gs) { return &gs->op[0]; }
...
static inline bool gimple_debug_bind_p(const gimple* s) {
    if (is_gimple_debug(s)) return s->subcode == GIMPLE_DEBUG_BIND;
...
static inline bool gimple_debug_source_bind_p(const gimple* s) {
    if (is_gimple_debug(s)) return s->subcode == GIMPLE_DEBUG_SOURCE_BIND;
...
static inline bool gimple_debug_begin_stmt_p(const gimple* s) {
    if (is_gimple_debug(s)) return s->subcode == GIMPLE_DEBUG_BEGIN_STMT;
...
static inline bool gimple_debug_inline_entry_p(const gimple* s) {
    if (is_gimple_debug(s)) return s->subcode == GIMPLE_DEBUG_INLINE_ENTRY;
...
static inline int get_lineno(const gimple* stmt) {
    ...
    if (loc == UNKNOWN_LOCATION) return -1;
...
static inline void gimple_omp_for_set_pre_body(gimple* gs,
                                               gimple_seq pre_body) {
    gomp_for* omp_for_stmt = as_a<gomp_for*>(gs);
...
static inline void gimple_omp_for_set_grid_phony(gomp_for* omp_for,
                                                 bool value) {
    gcc_checking_assert(gimple_omp_for_kind(omp_for) !=
                        GF_OMP_FOR_KIND_GRID_LOOP);
    if (value)
...
static inline tree* gimple_omp_parallel_clauses_ptr(
...
static inline void gimple_omp_parallel_set_clauses(
    gomp_parallel* omp_parallel_stmt, tree clauses) {
    omp_parallel_stmt->clauses = clauses;
}

/* Return the child function used to hold the body of OMP_PARALLEL_STMT.  */

static inline tree gimple_omp_parallel_child_fn(
...
static inline tree* gimple_omp_parallel_child_fn_ptr(
    gomp_parallel* omp_parallel_stmt) {
    return &omp_parallel_stmt->child_fn;
}
...
static inline void gimple_omp_parallel_set_child_fn(
    gomp_parallel* omp_parallel_stmt, tree child_fn) {
...
static inline tree gimple_omp_parallel_data_arg(
    const gomp_parallel* omp_parallel_stmt) {
...
static inline tree* gimple_omp_parallel_data_arg_ptr(
    gomp_parallel* omp_parallel_stmt) {
    return &omp_parallel_stmt->data_arg;
}
...
static inline void gimple_omp_parallel_set_data_arg(
    gomp_parallel* omp_parallel_stmt, tree data_arg) {
...
static inline tree gimple_omp_target_child_fn(
...
static inline tree* gimple_omp_target_child_fn_ptr(
...
static inline tree gimple_omp_target_data_arg(
    const gomp_target* omp_target_stmt) {
...
static inline tree* gimple_omp_target_data_arg_ptr(
    gomp_target* omp_target_stmt) {
    return &omp_target_stmt->data_arg;
}
...
static inline bool gimple_omp_teams_grid_phony(
...
static inline void gimple_omp_atomic_store_set_val(
    gomp_atomic_store* store_stmt, tree val) {
    store_stmt->val = val;
}

/* Return the value being stored in an atomic store.  */

static inline tree gimple_omp_atomic_store_val(
    const gomp_atomic_store* store_stmt) {
    return store_stmt->val;
}

/* Return a pointer to the value being stored in an atomic store.  */

static inline tree* gimple_omp_atomic_store_val_ptr(
    gomp_atomic_store* store_stmt) {
    return &store_stmt->val;
}
...
static inline tree gimple_omp_atomic_load_lhs(
    const gomp_atomic_load* load_stmt) {
    return load_stmt->lhs;
}

/* Return a pointer to the LHS of an atomic load.  */

static inline tree* gimple_omp_atomic_load_lhs_ptr(
    gomp_atomic_load* load_stmt) {
    return &load_stmt->lhs;
}
...
static inline tree gimple_omp_atomic_load_rhs(
    const gomp_atomic_load* load_stmt) {
...
static inline tree* gimple_omp_atomic_load_rhs_ptr(
    gomp_atomic_load* load_stmt) {
    return &load_stmt->rhs;
}
...
static inline tree gimple_omp_continue_control_def(
    const gomp_continue* cont_stmt) {
    return cont_stmt->control_def;
}

/* The same as above, but return the address.  */

static inline tree* gimple_omp_continue_control_def_ptr(
...
static inline tree gimple_omp_continue_control_use(
    const gomp_continue* cont_stmt) {
...
static inline tree* gimple_omp_continue_control_use_ptr(
    gomp_continue* cont_stmt) {
    return &cont_stmt->control_use;
}
...
static inline gimple_seq* gimple_transaction_body_ptr(
    gtransaction* transaction_stmt) {
    return &transaction_stmt->body;
}

/* Return the body for the GIMPLE_TRANSACTION statement TRANSACTION_STMT.  */

static inline gimple_seq gimple_transaction_body(
    gtransaction* transaction_stmt) {
    return transaction_stmt->body;
}
...
static inline tree gimple_transaction_label_norm(
    const gtransaction* transaction_stmt) {
    return transaction_stmt->label_norm;
}

static inline tree* gimple_transaction_label_norm_ptr(
    gtransaction* transaction_stmt) {
    return &transaction_stmt->label_norm;
}

static inline tree gimple_transaction_label_uninst(
    const gtransaction* transaction_stmt) {
    return transaction_stmt->label_uninst;
}

static inline tree* gimple_transaction_label_uninst_ptr(
    gtransaction* transaction_stmt) {
    return &transaction_stmt->label_uninst;
}

static inline tree gimple_transaction_label_over(
    const gtransaction* transaction_stmt) {
    return transaction_stmt->label_over;
}

static inline tree* gimple_transaction_label_over_ptr(
    gtransaction* transaction_stmt) {
    return &transaction_stmt->label_over;
}
...
static inline unsigned int gimple_transaction_subcode(
    const gtransaction* transaction_stmt) {
...
static inline void gimple_transaction_set_label_norm(
    gtransaction* transaction_stmt, tree label) {
    transaction_stmt->label_norm = label;
}

static inline void gimple_transaction_set_label_uninst(
    gtransaction* transaction_stmt, tree label) {
    transaction_stmt->label_uninst = label;
}

static inline void gimple_transaction_set_label_over(
    gtransaction* transaction_stmt, tree label) {
...
static inline void gimple_transaction_set_subcode(
    gtransaction* transaction_stmt, unsigned int subcode) {
    transaction_stmt->subcode = subcode;
}

/* Return a pointer to the return value for GIMPLE_RETURN GS.  */

static inline tree* gimple_return_retval_ptr(greturn* gs) { return &gs->op[0]; }

/* Return the return value for GIMPLE_RETURN GS.  */

static inline tree gimple_return_retval(const greturn* gs) { return gs->op[0]; }
...
static inline tree gimple_return_retbnd(const gimple* gs) {
...
static inline void gimple_return_set_retbnd(gimple* gs, tree retval) {
    GIMPLE_CHECK(gs, GIMPLE_RETURN);
