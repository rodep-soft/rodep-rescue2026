...
inline void symtab_node::checking_verify_symtab_nodes(void) {
    if (flag_checking) symtab_node::verify_symtab_nodes();
}
...
struct GTY((tag("SYMTAB_FUNCTION"))) cgraph_node : public symtab_node {
   public:
    ...
    cgraph_node* function_or_virtual_thunk_symbol(
        enum availability* avail = NULL, struct symtab_node* ref = NULL);
    ...
    cgraph_node* create_thunk(tree alias, tree, bool this_adjusting,
                              HOST_WIDE_INT fixed_offset,
                              HOST_WIDE_INT virtual_value, tree virtual_offset,
                              tree real_alias);
...
inline bool symtab_node::real_symbol_p(void) {
    ...
    if (DECL_ABSTRACT_P(decl)) return false;
    if (transparent_alias && definition) return false;
    if (!is_a<cgraph_node*>(this)) return true;
    cnode = dyn_cast<cgraph_node*>(this);
    if (cnode->global.inlined_to) return false;
    return true;
}
...
inline bool symtab_node::in_same_comdat_group_p(symtab_node* target) {
    ...
    if (cgraph_node* cn = dyn_cast<cgraph_node*>(target)) {
        if (cn->global.inlined_to) source = cn->global.inlined_to;
    }
    if (cgraph_node* cn = dyn_cast<cgraph_node*>(target)) {
        if (cn->global.inlined_to) target = cn->global.inlined_to;
    }
...
inline symtab_node* symtab_node::get_alias_target(void) {
    ...
    if (ref->use == IPA_REF_CHKP) iterate_reference(1, ref);
...
inline symtab_node* symtab_node::next_defined_symbol(void) {
    ...
    for (; node1; node1 = node1->next)
        if (node1->definition) return node1;
...
inline ipa_ref* symtab_node::iterate_direct_aliases(unsigned i, ipa_ref*& ref) {
    ...
    if (ref && ref->use != IPA_REF_ALIAS) return NULL;
...
inline bool symtab_node::used_from_object_file_p(void) {
    if (!TREE_PUBLIC(decl) || DECL_EXTERNAL(decl)) return false;
    if (resolution_used_from_other_file_p(resolution)) return true;
    return false;
}
...
inline void symbol_table::release_symbol(cgraph_node* node, int uid) {
    cgraph_count--;
...
inline varpool_node* symbol_table::first_static_initializer(void) {
    ...
    for (node = nodes; node; node = node->next) {
        ...
        if (vnode && DECL_INITIAL(node->decl)) return vnode;
    }
    return NULL;
}
...
inline varpool_node* symbol_table::next_static_initializer(varpool_node* node) {
    ...
    for (; node1; node1 = node1->next) {
        ...
        if (vnode1 && DECL_INITIAL(node1->decl)) return vnode1;
    }
    return NULL;
}
...
inline varpool_node* symbol_table::first_defined_variable(void) {
    ...
    for (node = nodes; node; node = node->next) {
        varpool_node* vnode = dyn_cast<varpool_node*>(node);
        if (vnode && vnode->definition) return vnode;
    }
    return NULL;
}
...
inline varpool_node* symbol_table::next_defined_variable(varpool_node* node) {
    ...
    for (; node1; node1 = node1->next) {
        varpool_node* vnode1 = dyn_cast<varpool_node*>(node1);
        if (vnode1 && vnode1->definition) return vnode1;
    }
    return NULL;
}
...
inline cgraph_node* symbol_table::next_defined_function(cgraph_node* node) {
    ...
    for (; node1; node1 = node1->next) {
        ...
        if (cn1 && cn1->definition) return cn1;
    }
    return NULL;
}
...
inline cgraph_node* symbol_table::first_function(void) {
    symtab_node* node;
    for (node = nodes; node; node = node->next)
        if (cgraph_node* cn = dyn_cast<cgraph_node*>(node)) return cn;
    return NULL;
}
...
inline cgraph_node* symbol_table::next_function(cgraph_node* node) {
    symtab_node* node1 = node->next;
    for (; node1; node1 = node1->next)
        if (cgraph_node* cn1 = dyn_cast<cgraph_node*>(node1)) return cn1;
    return NULL;
}
...
inline cgraph_node* symbol_table::first_function_with_gimple_body(void) {
    ...
    for (node = nodes; node; node = node->next) {
        cgraph_node* cn = dyn_cast<cgraph_node*>(node);
        if (cn && cn->has_gimple_body_p()) return cn;
    }
    return NULL;
}

/* Return next reachable static variable with initializer after NODE.  */
inline cgraph_node* symbol_table::next_function_with_gimple_body(
    cgraph_node* node) {
    symtab_node* node1 = node->next;
    for (; node1; node1 = node1->next) {
        cgraph_node* cn1 = dyn_cast<cgraph_node*>(node1);
        if (cn1 && cn1->has_gimple_body_p()) return cn1;
    }
    return NULL;
}
...
inline bool cgraph_node::can_remove_if_no_direct_calls_and_refs_p(void) {
    ...
    /* Instrumentation clones should not be removed before
       ...
       instrumentation.  */
    if (instrumentation_clone && !chkp_function_instrumented_p(decl))
        return false;
    /* Extern inlines can always go, we will use the external definition.  */
    if (DECL_EXTERNAL(decl)) return true;
    /* When function is needed, we can not remove it.  */
    if (force_output || used_from_other_partition) return false;
...
inline void cgraph_node::checking_verify_cgraph_nodes(void) {
    if (flag_checking) cgraph_node::verify_cgraph_nodes();
}
...
inline symtab_node* symtab_node::ultimate_alias_target(
    enum availability* availability, symtab_node* ref) {
    if (!alias) {
        if (availability) *availability = get_availability(ref);
        return this;
    }
...
inline cgraph_node* cgraph_node::ultimate_alias_target(
    enum availability* availability, symtab_node* ref) {
    cgraph_node* n = dyn_cast<cgraph_node*>(
        symtab_node::ultimate_alias_target(availability, ref));
    if (!n && availability) *availability = AVAIL_NOT_AVAILABLE;
    return n;
}
...
inline varpool_node* varpool_node::ultimate_alias_target(
    availability* availability, symtab_node* ref) {
...
inline void cgraph_edge::set_callee(cgraph_node* n) {
    prev_caller = NULL;
    if (n->callers) n->callers->prev_caller = this;
...
inline void cgraph_edge::remove_callee(void) {
    ...
    if (prev_caller) prev_caller->next_caller = next_caller;
    if (next_caller) next_caller->prev_caller = prev_caller;
    if (!prev_caller) callee->callers = next_caller;
}
...
inline bool cgraph_node::optimize_for_size_p(void) {
    if (opt_for_fn(decl, optimize_size)) return true;
...
inline bool symtab_node::call_for_symbol_and_aliases(
    ...
    if (include_overwritable || get_availability() > AVAIL_INTERPOSABLE) {
        if (callback(this, data)) return true;
    }
    if (has_aliases_p())
...
inline bool ipa_ref::address_matters_p() {
    if (use != IPA_REF_ADDR) return false;
...
inline void ipa_polymorphic_call_context::offset_by(HOST_WIDE_INT off) {
    if (outer_type) offset += off;
    if (speculative_outer_type) speculative_offset += off;
}
...
static inline bool cgraph_local_p(cgraph_node* node) {
