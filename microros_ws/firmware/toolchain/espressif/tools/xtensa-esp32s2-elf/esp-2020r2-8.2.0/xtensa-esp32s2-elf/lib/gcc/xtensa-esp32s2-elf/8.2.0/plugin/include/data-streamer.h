...
static inline void bp_pack_value(struct bitpack_d* bp, bitpack_word_t val,
                                 unsigned nbits) {
...
/* Pack VAL into the bit-packing context BP, using NBITS for each
   coefficient.  */
static inline void bp_pack_poly_value(
    struct bitpack_d* bp,
    const poly_int<NUM_POLY_INT_COEFFS, bitpack_word_t>& val, unsigned nbits) {
...
static inline struct bitpack_d streamer_read_bitpack(
...
static inline bitpack_word_t bp_unpack_value(struct bitpack_d* bp,
                                             unsigned nbits) {
...
static inline void streamer_write_char_stream(struct lto_output_stream* obs,
                                              char c) {
    /* No space left.  */
    if (obs->left_in_block == 0) lto_append_block(obs);
...
static inline unsigned char streamer_read_uchar(struct lto_input_block* ib) {
    if (ib->p >= ib->len) lto_section_overrun(ib);
    return (ib->data[ib->p++]);
}
...
static inline void streamer_write_hwi_in_range(struct lto_output_stream* obs,
                                               HOST_WIDE_INT min,
                                               HOST_WIDE_INT max,
                                               HOST_WIDE_INT val) {
...
static inline HOST_WIDE_INT streamer_read_hwi_in_range(
    struct lto_input_block* ib, const char* purpose, HOST_WIDE_INT min,
    HOST_WIDE_INT max) {
    HOST_WIDE_INT range = max - min;
    ...
    if (val < min || val > max) lto_value_range_error(purpose, val, min, max);
    return val;
}
...
static inline void bp_pack_int_in_range(struct bitpack_d* bp, HOST_WIDE_INT min,
...
static inline HOST_WIDE_INT bp_unpack_int_in_range(struct bitpack_d* bp,
                                                   const char* purpose,
                                                   HOST_WIDE_INT min,
                                                   HOST_WIDE_INT max) {
    HOST_WIDE_INT range = max - min;
...
static inline void streamer_write_record_start(struct output_block* ob,
                                               enum LTO_tags tag) {
...
static inline enum LTO_tags streamer_read_record_start(
    struct lto_input_block* ib) {
