...
struct hsa_symbol {
    ...
    /* Pointer to the original tree, which is PARM_DECL for input parameters and
       RESULT_DECL for the output parameters.  Also can be CONST_DECL for
       Fortran constants which need to be put into readonly segment.  */
    tree m_decl;
...
class hsa_op_base {
   public:
    ...
    /* Offset to which the associated operand structure will be written.  Zero
       if yet not scheduled for writing.  */
...
template <>
template <>
inline bool is_a_helper<hsa_op_immed*>::test(hsa_op_base* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_op_immed*>::test(hsa_op_with_type* p) {
    return p->m_kind == BRIG_KIND_OPERAND_CONSTANT_BYTES;
}
...
template <>
template <>
inline bool is_a_helper<hsa_op_reg*>::test(hsa_op_base* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_op_reg*>::test(hsa_op_with_type* p) {
    return p->m_kind == BRIG_KIND_OPERAND_REGISTER;
}
...
class hsa_op_address : public hsa_op_base {
   public:
    /* set up a new address operand consisting of base symbol SYM, register R
       and immediate OFFSET.  If the machine model is not large and offset is 64
       bit, the upper, 32 bits have to be zero.  */
    ...
    /* Set up a new address operand consisting of base symbol SYM and
       immediate OFFSET.  If the machine model is not large and offset is 64
       bit, the upper, 32 bits have to be zero.  */
...
template <>
template <>
inline bool is_a_helper<hsa_op_address*>::test(hsa_op_base* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_op_code_ref*>::test(hsa_op_base* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_op_code_list*>::test(hsa_op_base* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_op_operand_list*>::test(hsa_op_base* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_phi*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_br*>::test(hsa_insn_basic* p) {
...
class hsa_insn_cbr : public hsa_insn_br {
   ...
   private:
    /* Make the default constructor inaccessible.  */
    hsa_insn_cbr()
        : hsa_insn_br(0, BRIG_OPCODE_CBR, BRIG_TYPE_B1, BRIG_WIDTH_1) {}
};
...
template <>
template <>
inline bool is_a_helper<hsa_insn_cbr*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_sbr*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_cmp*>::test(hsa_insn_basic* p) {
...
class hsa_insn_mem : public hsa_insn_basic {
   public:
    ...
    /* The segment is of the memory access is either the segment of the symbol
       in the address operand or flat address is there is no symbol there.  */
...
template <>
template <>
inline bool is_a_helper<hsa_insn_mem*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_atomic*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_signal*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_seg*>::test(hsa_insn_basic* p) {
...
class hsa_internal_fn {
   public:
    ...
    hsa_internal_fn(const hsa_internal_fn* f)
        : m_fn(f->m_fn),
          m_type_bit_size(f->m_type_bit_size),
          m_offset(f->m_offset) {}
...
template <>
template <>
inline bool is_a_helper<hsa_insn_call*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_arg_block*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_comment*>::test(hsa_insn_basic* p) {
...
class hsa_insn_queue : public hsa_insn_basic {
   public:
    hsa_insn_queue(int nops, int opcode, BrigSegment segment,
                   BrigMemoryOrder memory_order, hsa_op_base* arg0 = NULL,
...
template <>
template <>
inline bool is_a_helper<hsa_insn_queue*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_srctype*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_packed*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_cvt*>::test(hsa_insn_basic* p) {
...
template <>
template <>
inline bool is_a_helper<hsa_insn_alloca*>::test(hsa_insn_basic* p) {
...
inline bool hsa_noop_symbol_hasher::equal(const value_type a,
...
enum hsa_function_kind { HSA_NONE, HSA_KERNEL, HSA_FUNCTION };
...
inline hsa_function_summary::hsa_function_summary()
    : m_kind(HSA_NONE), m_bound_function(NULL), m_gpu_implementation_p(false) {}
...
class hsa_summary_t : public function_summary<hsa_function_summary*> {
   public:
    ...
    /* Couple GPU and HOST as gpu-specific and host-specific implementation of
       ...
       kernel or gpu-callable function and GRIDIFIED_KERNEL_P is set if the
       function was gridified in OMP.  */
...
inline bool hsa_internal_fn_hasher::equal(const value_type a,
                                          const compare_type b) {
...
static inline bool hsa_gpu_implementation_p(tree decl) {
    if (hsa_summaries == NULL) return false;
