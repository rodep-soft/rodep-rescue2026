...
class sreal {
   public:
    ...
    /* Construct a sreal.  */
    sreal(int64_t sig, int exp = 0) : m_sig(sig), m_exp(exp) { normalize(); }
    ...
    bool operator<(const sreal& other) const {
        if (m_exp == other.m_exp)
            ...
            if (negative != other_negative) return negative > other_negative;
    ...
    sreal shift(int s) const {
        ...
        if (!m_sig) return *this;
        gcc_checking_assert(s <= SREAL_MAX_EXP);
        gcc_checking_assert(s >= -SREAL_MAX_EXP);

        /* Overflows/drop to 0 could be handled gracefully, but hopefully we do
           not need to do so.  */
   ...
   private:
    ...
    static sreal signedless_minus(const sreal& a, const sreal& b,
                                  bool negative);
...
inline void sreal::normalize_up() {
...
inline void sreal::normalize_down() {
    int last_bit;
    unsigned HOST_WIDE_INT sig = absu_hwi(m_sig);
