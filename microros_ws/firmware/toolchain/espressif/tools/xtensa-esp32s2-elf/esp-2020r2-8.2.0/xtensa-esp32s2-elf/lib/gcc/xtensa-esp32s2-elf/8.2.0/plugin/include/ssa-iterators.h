...
static inline void link_imm_use(ssa_use_operand_t* linknode, tree def) {
    ...
    if (!def || TREE_CODE(def) != SSA_NAME)
        ...
        if (linknode->use) gcc_checking_assert(*(linknode->use) == def);
...
static inline use_operand_p first_readonly_imm_use(imm_use_iterator* imm,
    ...
    if (end_readonly_imm_use_p(imm)) return NULL_USE_OPERAND_P;
    return imm->imm_use;
}
...
static inline bool has_zero_uses(const_tree var) {
    ...
    for (ptr = head->next; ptr != head; ptr = ptr->next)
        if (USE_STMT(ptr) && !is_gimple_debug(USE_STMT(ptr))) return false;
...
static inline unsigned int num_imm_uses(const_tree var) {
    ...
    if (!MAY_HAVE_DEBUG_BIND_STMTS) {
        ...
        for (ptr = start->next; ptr != start; ptr = ptr->next)
            if (USE_STMT(ptr) && !is_gimple_debug(USE_STMT(ptr))) num++;
...
static inline bool op_iter_done(const ssa_op_iter* ptr) { return ptr->done; }
...
static inline def_operand_p op_iter_next_def(ssa_op_iter* ptr) {
    ...
    if (ptr->flags & SSA_OP_DEF) {
        while (ptr->i < ptr->numops) {
            ...
            ptr->i++;
            if (*val) {
                if (TREE_CODE(*val) == TREE_LIST) val = &TREE_VALUE(*val);
...
static inline tree op_iter_next_tree(ssa_op_iter* ptr) {
    ...
    if (ptr->flags & SSA_OP_VDEF) {
        ptr->flags &= ~SSA_OP_VDEF;
        if ((val = gimple_vdef(ptr->stmt))) return val;
    }
    if (ptr->flags & SSA_OP_DEF) {
        while (ptr->i < ptr->numops) {
...
static inline tree op_iter_init_tree(ssa_op_iter* ptr, gimple* stmt,
                                     int flags) {
    op_iter_init(ptr, stmt, flags);
...
static inline tree single_ssa_tree_operand(gimple* stmt, int flags) {
    ...
    if (op_iter_done(&iter)) return NULL_TREE;
    op_iter_next_tree(&iter);
    if (op_iter_done(&iter)) return var;
    return NULL_TREE;
}
...
static inline use_operand_p single_ssa_use_operand(gimple* stmt, int flags) {
    ...
    if (op_iter_done(&iter)) return NULL_USE_OPERAND_P;
...
static inline use_operand_p ssa_vuse_operand(gimple* stmt) {
    if (!gimple_vuse(stmt)) return NULL_USE_OPERAND_P;
...
static inline def_operand_p single_ssa_def_operand(gimple* stmt, int flags) {
    ...
    if (op_iter_done(&iter)) return NULL_DEF_OPERAND_P;
...
static inline tree single_phi_def(gphi* stmt, int flags) {
    ...
    if ((flags & SSA_OP_DEF) && is_gimple_reg(def)) return def;
    if ((flags & SSA_OP_VIRTUAL_DEFS) && !is_gimple_reg(def)) return def;
    return NULL_TREE;
}
...
static inline void link_use_stmts_after(use_operand_p head,
    ...
    if (imm->iter_node.prev != NULL) delink_imm_use(&imm->iter_node);
...
static inline gimple* first_imm_use_stmt(imm_use_iterator* imm, tree var) {
    ...
    if (end_imm_use_stmt_p(imm)) return NULL;
