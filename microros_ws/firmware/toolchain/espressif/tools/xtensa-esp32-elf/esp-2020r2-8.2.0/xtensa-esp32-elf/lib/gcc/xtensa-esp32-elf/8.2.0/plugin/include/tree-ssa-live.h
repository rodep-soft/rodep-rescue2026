...
static inline tree partition_to_var(var_map map, int i) {
    tree name;
    if (map->view_to_partition) i = map->view_to_partition[i];
...
static inline tree version_to_var(var_map map, int version) {
    ...
    if (map->partition_to_view) part = map->partition_to_view[part];
    if (part == NO_PARTITION) return NULL_TREE;
...
static inline int num_basevars(var_map map) { return map->num_basevars; }
...
static inline var_map live_var_map(tree_live_info_p live) { return live->map; }

/* Merge the live on entry information in LIVE for partitions P1 and P2. Place
   the result into P1.  Clear P2.  */

static inline void live_merge_and_clear(tree_live_info_p live, int p1, int p2) {
...
static inline void make_live_on_entry(tree_live_info_p live, basic_block bb,
                                      int p) {
