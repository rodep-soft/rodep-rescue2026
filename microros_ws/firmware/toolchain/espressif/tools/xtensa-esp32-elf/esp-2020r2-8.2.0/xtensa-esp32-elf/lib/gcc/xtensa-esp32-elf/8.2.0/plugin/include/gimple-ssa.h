...
static inline use_operand_p gimple_vuse_op(const gimple* g) {
    ...
    if (!mem_ops_stmt) return NULL_USE_OPERAND_P;
...
static inline def_operand_p gimple_vdef_op(gimple* g) {
    ...
    if (!mem_ops_stmt) return NULL_DEF_OPERAND_P;
    if (mem_ops_stmt->vdef) return &mem_ops_stmt->vdef;
...
static inline void update_stmt_if_modified(gimple* s) {
    if (gimple_modified_p(s)) update_stmt_operands(cfun, s);
}
