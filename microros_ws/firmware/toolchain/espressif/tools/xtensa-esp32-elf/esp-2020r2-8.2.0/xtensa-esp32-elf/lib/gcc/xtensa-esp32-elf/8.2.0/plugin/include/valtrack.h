...
/* Hash on the pseudo number.  */
inline hashval_t dead_debug_hash_descr::hash(
    const dead_debug_global_entry* my) {
    return REGNO(my->reg);
}
...
extern int dead_debug_insert_temp(struct dead_debug_local*, unsigned int uregno,
                                  rtx_insn* insn, enum debug_temp_where);
