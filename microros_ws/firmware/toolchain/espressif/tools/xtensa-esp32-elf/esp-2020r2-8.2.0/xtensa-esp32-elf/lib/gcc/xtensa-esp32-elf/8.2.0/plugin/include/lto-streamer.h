...
#define DEFINE_DECL_STREAM_FUNCS(UPPER_NAME, name)                            \
    static inline tree lto_file_decl_data_get_##name(                         \
        struct lto_file_decl_data* data, unsigned int idx) {                  \
...
class lto_location_cache {
   ...
   private:
    ...
    /* Bookkeeping to remember state in between calls to
       lto_apply_location_cache When streaming gimple, the location cache is not
       used and thus lto_apply_location_cache happens per location basis.  It is
       then useful to avoid redundant calls of linemap API.  */
...
inline hashval_t string_slot_hasher::hash(const string_slot* ds) {
    ...
    for (i = 0; i < ds->len; i++) r = r * 67 + (unsigned)ds->s[i] - 113;
    return r;
}
...
inline bool string_slot_hasher::equal(const string_slot* ds1,
    ...
    if (ds1->len == ds2->len) return memcmp(ds1->s, ds2->s, ds1->len) == 0;
...
extern struct lto_input_block* lto_create_simple_input_block(
    struct lto_file_decl_data*, enum lto_section_type, const char**, size_t*);
...
extern void lto_set_in_hooks(struct lto_file_decl_data**,
                             lto_get_section_data_f*, lto_free_section_data_f*);
...
extern bool lto_output_decl_index(struct lto_output_stream*,
                                  struct lto_tree_ref_encoder*, tree,
                                  unsigned int*);
...
hashval_t lto_input_scc(struct lto_input_block*, struct data_in*, unsigned*,
                        unsigned*);
...
void cl_target_option_stream_in(struct data_in*, struct bitpack_d*,
                                struct cl_target_option*);
...
static inline void lto_tag_check_range(enum LTO_tags actual, enum LTO_tags tag1,
                                       enum LTO_tags tag2) {
...
static inline void lto_init_tree_ref_encoder(
...
static inline void lto_destroy_tree_ref_encoder(
    struct lto_tree_ref_encoder* encoder) {
...
static inline unsigned int lto_tree_ref_encoder_size(
    struct lto_tree_ref_encoder* encoder) {
    return encoder->trees.length();
}

/* Return the IDX-th tree in ENCODER. */
static inline tree lto_tree_ref_encoder_get_tree(
    struct lto_tree_ref_encoder* encoder, unsigned int idx) {
...
static inline int lto_symtab_encoder_lookup(lto_symtab_encoder_t encoder,
                                            symtab_node* node) {
...
/* Return the node pointed to by LSI.  */
static inline struct cgraph_node* lsei_cgraph_node(
    lto_symtab_encoder_iterator lsei) {
    return dyn_cast<cgraph_node*>(lsei.encoder->nodes[lsei.index].node);
}

/* Return the node pointed to by LSI.  */
static inline varpool_node* lsei_varpool_node(
    lto_symtab_encoder_iterator lsei) {
...
static inline symtab_node* lto_symtab_encoder_deref(
    lto_symtab_encoder_t encoder, int ref) {
    if (ref == LCC_NOT_FOUND) return NULL;

    return encoder->nodes[ref].node;
}

/* Return an iterator to the first node in LSI.  */
static inline lto_symtab_encoder_iterator lsei_start(
    lto_symtab_encoder_t encoder) {
    lto_symtab_encoder_iterator lsei;
...
/* Return an iterator to the first node in LSI.  */
static inline lto_symtab_encoder_iterator lsei_start_in_partition(
    lto_symtab_encoder_t encoder) {
...
/* Advance iterator LSE.  */
static inline void lsei_next_function_in_partition(
    lto_symtab_encoder_iterator* lsei) {
    lsei_next(lsei);
    while (!lsei_end_p(*lsei) && (!is_a<cgraph_node*>(lsei_node(*lsei)) ||
                                  !lto_symtab_encoder_in_partition_p(
                                      lsei->encoder, lsei_node(*lsei))))
        lsei_next(lsei);
}

/* Return an iterator to the first node in LSI.  */
static inline lto_symtab_encoder_iterator lsei_start_function_in_partition(
    lto_symtab_encoder_t encoder) {
    lto_symtab_encoder_iterator lsei = lsei_start(encoder);
...
/* Advance iterator LSE.  */
static inline void lsei_next_variable_in_partition(
    lto_symtab_encoder_iterator* lsei) {
    lsei_next(lsei);
    while (!lsei_end_p(*lsei) && (!is_a<varpool_node*>(lsei_node(*lsei)) ||
                                  !lto_symtab_encoder_in_partition_p(
                                      lsei->encoder, lsei_node(*lsei))))
        lsei_next(lsei);
}
...
/* Return an iterator to the first node in LSI.  */
static inline lto_symtab_encoder_iterator lsei_start_variable_in_partition(
    lto_symtab_encoder_t encoder) {
    lto_symtab_encoder_iterator lsei = lsei_start(encoder);
