...
#define OMP_CLAUSE_SIZE(NODE)                                           \
    OMP_CLAUSE_OPERAND(                                                 \
        OMP_CLAUSE_RANGE_CHECK(OMP_CLAUSE_CHECK(NODE), OMP_CLAUSE_FROM, \
                               OMP_CLAUSE__CACHE_),                     \
        1)
...
#define OMP_CLAUSE_DECL(NODE)                                              \
    OMP_CLAUSE_OPERAND(                                                    \
        OMP_CLAUSE_RANGE_CHECK(OMP_CLAUSE_CHECK(NODE), OMP_CLAUSE_PRIVATE, \
...
#define OMP_CLAUSE_REDUCTION_CODE(NODE)                   \
    (OMP_CLAUSE_SUBCODE_CHECK(NODE, OMP_CLAUSE_REDUCTION) \
...
inline tree tree_check(tree __t, const char* __f, int __l, const char* __g,
                       tree_code __c) {
    if (TREE_CODE(__t) != __c) tree_check_failed(__t, __f, __l, __g, __c, 0);
    return __t;
}
...
inline tree non_type_check(tree __t, const char* __f, int __l,
                           const char* __g) {
    if (TYPE_P(__t)) tree_not_class_check_failed(__t, tcc_type, __f, __l, __g);
    return __t;
}
...
inline poly_uint64 TYPE_VECTOR_SUBPARTS(const_tree node) {
    ...
    if (NUM_POLY_INT_COEFFS == 2) {
        ...
        if (precision & 0x100) res.coeffs[1] = 1 << (precision & 0xff);
        return res;
    } else
        return 1 << precision;
}
...
static inline bool valid_vector_subparts_p(poly_uint64 subparts) {
    unsigned HOST_WIDE_INT coeff0 = subparts.coeffs[0];
    if (!pow2p_hwi(coeff0)) return false;
    if (NUM_POLY_INT_COEFFS == 2) {
        unsigned HOST_WIDE_INT coeff1 = subparts.coeffs[1];
        if (coeff1 != 0 && coeff1 != coeff0) return false;
    }
    return true;
}
...
static inline tree build1_loc(location_t loc, enum tree_code code, tree type,
    ...
    if (CAN_HAVE_LOCATION_P(t)) SET_EXPR_LOCATION(t, loc);
    return t;
}
...
#define FIND_PLACEHOLDER_IN_EXPR(EXP, V)                                    \
    do {                                                                    \
        if ((EXP) && !TREE_CONSTANT(EXP)) find_placeholder_in_expr(EXP, V); \
    } while (0)
...
static inline bool storage_order_barrier_p(const_tree t) {
    if (TREE_CODE(t) != VIEW_CONVERT_EXPR) return false;
...
extern bool gimple_canonical_types_compatible_p(
    const_tree, const_tree, bool trust_type_canonical = true);
...
/* Return simplified tree code of type that is used for canonical type
   merging.  */
inline enum tree_code tree_code_for_canonical_type_merging(
    enum tree_code code) {
    ...
    if (code == ENUMERAL_TYPE) return INTEGER_TYPE;
    ...
    if (code == REFERENCE_TYPE) return POINTER_TYPE;
    return code;
}
...
static inline void init_const_call_expr_arg_iterator(
...
static inline const_tree next_const_call_expr_arg(
...
static inline const_tree first_const_call_expr_arg(
    const_tree exp, const_call_expr_arg_iterator* iter) {
...
static inline tree builtin_decl_implicit(enum built_in_function fncode) {
    ...
    if (!builtin_info[uns_fncode].implicit_p) return NULL_TREE;
...
static inline bool may_be_aliased(const_tree var) {
    return (
        ...
        !((TREE_STATIC(var) || TREE_PUBLIC(var) || DECL_EXTERNAL(var)) &&
          ((TREE_READONLY(var) && !TYPE_NEEDS_CONSTRUCTING(TREE_TYPE(var))) ||
...
static inline struct cl_optimization* opts_for_fn(const_tree fndecl) {
    ...
    if (fn_opts == NULL_TREE) fn_opts = optimization_default_node;
...
static inline cl_target_option* target_opts_for_fn(const_tree fndecl) {
    ...
    if (fn_opts == NULL_TREE) fn_opts = target_option_default_node;
...
inline wi::tree_to_poly_wide_ref wi::to_poly_wide(const_tree t) {
    if (POLY_INT_CST_P(t)) return poly_int_cst_value(t);
    return t;
}
...
bool wi::fits_to_tree_p(const T& x, const_tree type) {
    ...
    if (TREE_CODE(type) == BOOLEAN_TYPE) return fits_to_boolean_p(x, type);
...
static inline source_range get_decl_source_range(tree decl) {
...
inline bool desired_pro_or_demotion_p(const_tree to_type,
    ...
    if (to_type_precision <= GET_MODE_PRECISION(word_mode)) return true;
