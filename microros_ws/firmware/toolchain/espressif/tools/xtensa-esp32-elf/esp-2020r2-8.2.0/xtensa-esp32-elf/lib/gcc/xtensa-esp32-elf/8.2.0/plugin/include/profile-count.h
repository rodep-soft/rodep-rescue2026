...
enum profile_quality {
    ...
    /* Profile is based on static branch prediction heuristics and may
       ...
       compared inter-procedurally.  Never used by probabilities (they are
       always local).
     */
...
class GTY((user)) profile_probability {
   ...
   public:
    ...
    bool reliable_p() const { return m_quality >= profile_adjusted; }
    ...
    profile_probability operator+(const profile_probability& other) const {
        if (other == profile_probability::never()) return *this;
        if (*this == profile_probability::never()) return other;
    ...
    gcov_type apply(gcov_type val) const {
        if (*this == profile_probability::uninitialized()) return val / 2;
    ...
    profile_probability apply_scale(int64_t num, int64_t den) const {
        if (*this == profile_probability::never()) return *this;
        if (!initialized_p()) return profile_probability::uninitialized();
        profile_probability ret;
        uint64_t tmp;
    ...
    /* Return true when the probability of edge is reliable.
       ...
       information across the CFG if we find this information useful on multiple
       places.   */

    bool probably_reliable_p() const {
        if (m_quality >= profile_adjusted) return true;
        if (!initialized_p()) return false;
...
class GTY(()) profile_count {
   ...
   private:
    ...
    bool compatible_p(const profile_count other) const {
        if (!initialized_p() || !other.initialized_p()) return true;
   ...
   public:
    ...
    bool precise_p() const { return m_quality == profile_precise; }
    ...
    profile_count merge(profile_count other) const {
        if (*this == other || !other.initialized_p() ||
            ...
            return *this;
        if (other.m_quality > m_quality || other > *this) return other;
        return *this;
    }
    ...
    profile_count operator+(const profile_count& other) const {
        if (other == profile_count::zero()) return *this;
        if (*this == profile_count::zero()) return other;
        if (!initialized_p() || !other.initialized_p())
    ...
    /* Comparsions are three-state and conservative.  False is returned if
       the inequality can not be decided.  */
    bool operator<(const profile_count& other) const {
        if (!initialized_p() || !other.initialized_p()) return false;
        if (*this == profile_count::zero())
            return !(other == profile_count::zero());
        if (other == profile_count::zero()) return false;
        gcc_checking_assert(compatible_p(other));
        return m_val < other.m_val;
    }
    bool operator>(const profile_count& other) const {
        if (!initialized_p() || !other.initialized_p()) return false;
        if (*this == profile_count::zero()) return false;
    ...
    bool operator<=(const profile_count& other) const {
        if (!initialized_p() || !other.initialized_p()) return false;
        if (*this == profile_count::zero()) return true;
        if (other == profile_count::zero())
            return (*this == profile_count::zero());
        gcc_checking_assert(compatible_p(other));
        return m_val <= other.m_val;
    ...
    bool operator>=(const profile_count& other) const {
        if (!initialized_p() || !other.initialized_p()) return false;
        if (other == profile_count::zero()) return true;
        if (*this == profile_count::zero())
            return !(other == profile_count::zero());
        gcc_checking_assert(compatible_p(other));
        return m_val >= other.m_val;
    ...
    profile_count max(profile_count other) const {
        if (!initialized_p()) return other;
        if (!other.initialized_p()) return *this;
        if (*this == profile_count::zero()) return other;
        if (other == profile_count::zero()) return *this;
        gcc_checking_assert(compatible_p(other));
        if (m_val < other.m_val ||
    ...
    profile_count apply_probability(int prob) const {
        ...
        if (!initialized_p()) return profile_count::uninitialized();
        profile_count ret;
    ...
    profile_count apply_probability(profile_probability prob) const {
        if (*this == profile_count::zero()) return *this;
        if (prob == profile_probability::never()) return profile_count::zero();
        if (!initialized_p()) return profile_count::uninitialized();
        profile_count ret;
        uint64_t tmp;
    ...
    profile_count apply_scale(profile_count num, profile_count den) const {
        if (*this == profile_count::zero()) return *this;
        if (num == profile_count::zero()) return num;
    ...
    profile_count ipa() const {
        if (m_quality > profile_guessed_global0adjusted) return *this;
        if (m_quality == profile_guessed_global0) return profile_count::zero();
