...
static inline bool gsi_end_p(gimple_stmt_iterator i) { return i.ptr == NULL; }
...
static inline void gsi_next(gimple_stmt_iterator* i) { i->ptr = i->ptr->next; }
...
static inline gimple* gsi_stmt(gimple_stmt_iterator i) { return i.ptr; }
...
static inline gimple_stmt_iterator gsi_start_nondebug_bb(basic_block bb) {
    ...
    if (!gsi_end_p(i) && is_gimple_debug(gsi_stmt(i))) gsi_next_nondebug(&i);
...
static inline gimple_stmt_iterator gsi_start_nondebug_after_labels_bb(
    basic_block bb) {
...
static inline gimple_stmt_iterator gsi_last_nondebug_bb(basic_block bb) {
    ...
    if (!gsi_end_p(i) && is_gimple_debug(gsi_stmt(i))) gsi_prev_nondebug(&i);
...
static inline bool gsi_one_nondebug_before_end_p(gimple_stmt_iterator i) {
    if (gsi_one_before_end_p(i)) return true;
    if (gsi_end_p(i)) return false;
    gsi_next_nondebug(&i);
    return gsi_end_p(i);
}
...
static inline basic_block gsi_bb(gimple_stmt_iterator i) { return i.bb; }

/* Return the sequence associated with this iterator.  */

static inline gimple_seq gsi_seq(gimple_stmt_iterator i) { return *i.seq; }
...
static inline bool gimple_seq_nondebug_singleton_p(gimple_seq seq) {
    ...
    gsi.seq = &seq;
    gsi.bb = NULL;
    while (!gsi_end_p(gsi) && is_gimple_debug(gsi_stmt(gsi))) gsi_next(&gsi);
