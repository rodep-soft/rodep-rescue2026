...
static inline struct ipa_cst_ref_desc* ipa_get_jf_constant_rdesc(
...
static inline tree ipa_get_jf_pass_through_operand(
    struct ipa_jump_func* jfunc) {
...
static inline int ipa_get_jf_pass_through_formal_id(
    struct ipa_jump_func* jfunc) {
    gcc_checking_assert(jfunc->type == IPA_JF_PASS_THROUGH);
...
static inline enum tree_code ipa_get_jf_pass_through_operation(
    struct ipa_jump_func* jfunc) {
    gcc_checking_assert(jfunc->type == IPA_JF_PASS_THROUGH);
...
static inline bool ipa_get_jf_pass_through_agg_preserved(
    struct ipa_jump_func* jfunc) {
    gcc_checking_assert(jfunc->type == IPA_JF_PASS_THROUGH);
...
static inline HOST_WIDE_INT ipa_get_jf_ancestor_offset(
    struct ipa_jump_func* jfunc) {
...
static inline bool ipa_get_jf_ancestor_agg_preserved(
    struct ipa_jump_func* jfunc) {
    gcc_checking_assert(jfunc->type == IPA_JF_ANCESTOR);
...
static inline bool ipa_get_jf_ancestor_type_preserved(
    struct ipa_jump_func* jfunc) {
    gcc_checking_assert(jfunc->type == IPA_JF_ANCESTOR);
    return jfunc->value.ancestor.agg_preserved;
}
...
static inline tree ipa_get_type(struct ipa_node_params* info, int i) {
    if (vec_safe_length(info->descriptors) <= (unsigned)i) return NULL;
    tree t = (*info->descriptors)[i].decl_or_type;
    if (!t) return NULL;
    if (TYPE_P(t)) return t;
    gcc_checking_assert(TREE_CODE(t) == PARM_DECL);
    return TREE_TYPE(t);
}
...
static inline struct ipa_jump_func* ipa_get_ith_jump_func(
    struct ipa_edge_args* args, int i) {
...
ipa_get_ith_polymorhic_call_context(struct ipa_edge_args* args, int i) {
    if (!args->polymorphic_call_contexts) return NULL;
...
static inline bool ipa_edge_args_info_available_for_edge_p(
    struct cgraph_edge* edge) {
    return ipa_edge_args_sum->exists(edge);
}

static inline ipcp_transformation_summary* ipcp_get_transformation_summary(
    cgraph_node* node) {
