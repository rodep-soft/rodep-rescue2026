...
struct RobotParameters {
    ...
    double meters_per_count() const {
        return (2.0 * M_PI * wheel_radius) / total_counts_per_rev();
    }
...
class DifferentialDriveController {
   public:
    explicit DifferentialDriveController(const RobotParameters& params)
        : params_(params) {}
   ...
   public:
    ...
    void update(int32_t left_counts, int32_t right_counts, double dt) {
        ...
        const double delta_theta =
            (right_distance - left_distance) / params_.wheel_base;
...
class CrawlerDriverV3 : public rclcpp::Node {
   public:
    CrawlerDriverV3()
        : Node("crawler_driver_v3"), estop_active_(false), initialized_(false) {
        ...
        // コントローラー初期化
        controller_ =
            std::make_unique<DifferentialDriveController>(robot_params_);
        ...
        roboclaw_ = std::make_unique<RoboclawDriver>(
            port, static_cast<uint8_t>(address));

        // Subscriber
        twist_sub_ = create_subscription<geometry_msgs::msg::Twist>(
            "cmd_vel", 10,
            std::bind(&CrawlerDriverV3::twistCallback, this,
                      std::placeholders::_1));

        estop_sub_ = create_subscription<std_msgs::msg::Bool>(
            "emergency_stop", 10,
            std::bind(&CrawlerDriverV3::estopCallback, this,
                      std::placeholders::_1));
   ...
   private:
    ...
    void loadParameters() {
        ...
        RCLCPP_INFO(get_logger(), "  wheel_base: %.3f m",
                    robot_params_.wheel_base);
        RCLCPP_INFO(get_logger(), "  wheel_radius: %.3f m",
                    robot_params_.wheel_radius);
        RCLCPP_INFO(get_logger(), "  total_reduction: %d",
                    robot_params_.total_counts_per_rev());
        RCLCPP_INFO(get_logger(), "  counts/meter: %.2f",
                    robot_params_.counts_per_meter());
        RCLCPP_INFO(get_logger(), "  meters/count: %.6f",
                    robot_params_.meters_per_count());
    }
    ...
    void initialize() {
        ...
        roboclaw_->setVelocity(Motor::M1, 0.0, [this](bool success) {
            if (success) RCLCPP_INFO(get_logger(), "  M1: 停止 ✓");
        });

        roboclaw_->setVelocity(Motor::M2, 0.0, [this](bool success) {
            if (success) RCLCPP_INFO(get_logger(), "  M2: 停止 ✓");
        });

        // PID設定
        roboclaw_->setPID(Motor::M1, m1_pid_, [this](bool success) {
            if (success) RCLCPP_INFO(get_logger(), "  M1 PID: 設定完了 ✓");
        });

        roboclaw_->setPID(Motor::M2, m2_pid_, [this](bool success) {
            if (success) RCLCPP_INFO(get_logger(), "  M2 PID: 設定完了 ✓");
        });
    ...
    void twistCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
        ...
        auto [left_vel, right_vel] =
            controller_->twistToWheelVelocities(msg->linear.x, msg->angular.z);
        ...
        RCLCPP_DEBUG(
            get_logger(),
            "cmd_vel: linear=%.2f angular=%.2f → L=%.0f R=%.0f counts/s",
