...
#define RANK(X) poly_coeff_traits<X>::rank
    static const int result_kind =
        ((RANK(T1) <= RANK(HOST_WIDE_INT) && RANK(T2) <= RANK(HOST_WIDE_INT))
             ? 0
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int_pod<N, C>& poly_int_pod<N, C>::operator=(
    const poly_int_pod<N, Ca>& a) {
...
inline poly_int_pod<N, C>& poly_int_pod<N, C>::operator+=(
    const poly_int_pod<N, Ca>& a) {
    for (unsigned int i = 0; i < N; i++) this->coeffs[i] += a.coeffs[i];
    return *this;
}
...
inline typename if_nonpoly<Ca, poly_int_pod<N, C> >::type&
poly_int_pod<N, C>::operator*=(const Ca& a) {
    for (unsigned int i = 0; i < N; i++) this->coeffs[i] *= a;
    return *this;
}
...
template <unsigned int N, typename C>
inline bool poly_int_pod<N, C>::is_constant() const {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
...
template <unsigned int N, typename C>
template <typename T>
inline typename if_lossless<T, C, bool>::type poly_int_pod<N, C>::is_constant(
    T* const_value) const {
    if (is_constant()) {
...
template <unsigned int N, typename C>
inline C poly_int_pod<N, C>::to_constant() const {
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C> poly_int_pod<N, C>::from(const poly_int_pod<N, Ca>& a,
...
template <unsigned int N, typename C>
inline bool poly_int_pod<N, C>::to_shwi(
    poly_int_pod<N, HOST_WIDE_INT>* r) const {
    for (unsigned int i = 0; i < N; i++)
        if (!wi::fits_shwi_p(this->coeffs[i])) return false;
    for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename C>
inline bool poly_int_pod<N, C>::to_uhwi(
    poly_int_pod<N, unsigned HOST_WIDE_INT>* r) const {
    for (unsigned int i = 0; i < N; i++)
        if (!wi::fits_uhwi_p(this->coeffs[i])) return false;
    for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename C>
inline poly_int<N, unsigned HOST_WIDE_INT> poly_int_pod<N, C>::force_uhwi()
    const {
...
template <unsigned int N, typename C>
inline poly_int_pod<N, C>::operator C() const {
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C>& poly_int<N, C>::operator+=(
    const poly_int_pod<N, Ca>& a) {
    for (unsigned int i = 0; i < N; i++) this->coeffs[i] += a.coeffs[i];
    return *this;
}
...
inline typename if_nonpoly<Ca, bool>::type coeffs_in_range_p(const Ca& a,
                                                             const Cb& b,
                                                             const Cc& c) {
    return a >= b && a <= c;
}
...
inline typename if_nonpoly<Ca, bool>::type coeffs_in_range_p(
    const poly_int_pod<N, Ca>& a, const Cb& b, const Cc& c) {
    for (unsigned int i = 0; i < N; i++)
        if (a.coeffs[i] < b || a.coeffs[i] > c) return false;
    return true;
}
...
template <unsigned int N>
inline poly_int<N, hwi_with_prec> uhwi(
    const poly_int_pod<N, unsigned HOST_WIDE_INT>& a, unsigned int precision) {
    poly_int<N, hwi_with_prec> r;
    for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) operator+(const poly_int_pod<N, Ca>& a,
                                              const Cb & b) {
    typedef POLY_CAST(Ca, Cb) NCa;
...
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const poly_int_pod<N, Ca>& a,
    ...
    for (unsigned int i = 1; i < N; i++)
        POLY_SET_COEFF(C, r, i,
                       wi::add(a.coeffs[i], wi::ints_for<Cb>::zero(b)));
    return r;
}
...
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const Ca& a,
    ...
    for (unsigned int i = 1; i < N; i++)
        POLY_SET_COEFF(C, r, i,
                       wi::add(wi::ints_for<Ca>::zero(a), b.coeffs[i]));
    return r;
}
...
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> sub(const poly_int_pod<N, Ca>& a,
    ...
    for (unsigned int i = 1; i < N; i++)
        POLY_SET_COEFF(C, r, i,
                       wi::sub(a.coeffs[i], wi::ints_for<Cb>::zero(b)));
    return r;
}
...
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> sub(const Ca& a,
    ...
    for (unsigned int i = 1; i < N; i++)
        POLY_SET_COEFF(C, r, i,
                       wi::sub(wi::ints_for<Ca>::zero(a), b.coeffs[i]));
    return r;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> mul(const poly_int_pod<N, Ca>& a,
                                                 const Cb& b) {
    typedef WI_BINARY_RESULT(Ca, Cb) C;
    poly_int<N, C> r;
    for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Ca)> lshift(
    const poly_int_pod<N, Ca>& a, const Cb& b) {
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_eq(const poly_int_pod<N, Ca>& a,
                     const poly_int_pod<N, Cb>& b) {
    STATIC_ASSERT(N <= 2);
    if (N == 2)
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_eq(
    const poly_int_pod<N, Ca>& a, const Cb& b) {
    STATIC_ASSERT(N <= 2);
    if (N == 2) return maybe_eq_2(a.coeffs[0], a.coeffs[1], b);
    return a.coeffs[0] == b;
}

template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_eq(
    const Ca& a, const poly_int_pod<N, Cb>& b) {
    STATIC_ASSERT(N <= 2);
    if (N == 2) return maybe_eq_2(b.coeffs[0], b.coeffs[1], a);
    return a == b.coeffs[0];
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_ne(const poly_int_pod<N, Ca>& a,
                     const poly_int_pod<N, Cb>& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (a.coeffs[i] != b.coeffs[i]) return true;
    return a.coeffs[0] != b.coeffs[0];
}

template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_ne(
    const poly_int_pod<N, Ca>& a, const Cb& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (a.coeffs[i] != 0) return true;
    return a.coeffs[0] != b;
}

template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_ne(
    const Ca& a, const poly_int_pod<N, Cb>& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (b.coeffs[i] != 0) return true;
    return a != b.coeffs[0];
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_le(const poly_int_pod<N, Ca>& a,
                     const poly_int_pod<N, Cb>& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (a.coeffs[i] < b.coeffs[i]) return true;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_le(
    const poly_int_pod<N, Ca>& a, const Cb& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (a.coeffs[i] < 0) return true;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_le(
    const Ca& a, const poly_int_pod<N, Cb>& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (b.coeffs[i] > 0) return true;
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_lt(const poly_int_pod<N, Ca>& a,
                     const poly_int_pod<N, Cb>& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (a.coeffs[i] < b.coeffs[i]) return true;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_lt(
    const poly_int_pod<N, Ca>& a, const Cb& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (a.coeffs[i] < 0) return true;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_lt(
    const Ca& a, const poly_int_pod<N, Cb>& b) {
    if (N >= 2)
        for (unsigned int i = 1; i < N; i++)
            if (b.coeffs[i] > 0) return true;
...
inline POLY_POLY_RESULT(N, Ca, Cb)
    ...
    if (known_le(a, b))
        ...
        if (N > 1) gcc_checking_assert(known_le(b, a));
        return b;
    }
}
...
template <unsigned int N, typename Ca>
inline Ca constant_lower_bound(const poly_int_pod<N, Ca>& a) {
...
inline POLY_BINARY_COEFF(Ca, Ca) coeff_gcd(const poly_int_pod<N, Ca>& a) {
    ...
    unsigned int i;
    for (i = N - 1; i > 0; --i)
        if (a.coeffs[i] != 0) break;
    typedef POLY_BINARY_COEFF(Ca, Ca) C;
    C r = a.coeffs[i];
    for (unsigned int j = 0; j < i; ++j)
        if (a.coeffs[j] != 0) r = gcd(r, C(a.coeffs[j]));
    return r;
}
...
force_common_multiple(const poly_int_pod<N, Ca>& a,
                      const poly_int_pod<N, Cb>& b) {
    if (b.is_constant()) return common_multiple(a, b.coeffs[0]);
    if (a.is_constant()) return common_multiple(a.coeffs[0], b);
...
template <unsigned int N, typename Ca, typename Cb>
inline int compare_sizes_for_sort(const poly_int_pod<N, Ca>& a,
                                  const poly_int_pod<N, Cb>& b) {
...
inline bool can_align_p(const poly_int_pod<N, Ca>& value, Cb align) {
    for (unsigned int i = 1; i < N; i++)
        if ((value.coeffs[i] & (align - 1)) != 0) return false;
    return true;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_up(const poly_int_pod<N, Ca>& value, Cb align,
                         poly_int_pod<N, Ca>* aligned) {
    if (!can_align_p(value, align)) return false;
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_down(const poly_int_pod<N, Ca>& value, Cb align,
                           poly_int_pod<N, Ca>* aligned) {
    if (!can_align_p(value, align)) return false;
    *aligned = value - (value.coeffs[0] & (align - 1));
    return true;
...
template <unsigned int N, typename Ca, typename Cb, typename Cc>
inline bool known_equal_after_align_up(const poly_int_pod<N, Ca>& a,
                                       const poly_int_pod<N, Cb>& b, Cc align) {
...
template <unsigned int N, typename Ca, typename Cb, typename Cc>
inline bool known_equal_after_align_down(const poly_int_pod<N, Ca>& a,
                                         const poly_int_pod<N, Cb>& b,
                                         Cc align) {
    poly_int<N, Ca> aligned_a;
    poly_int<N, Cb> aligned_b;
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_up(const poly_int_pod<N, Ca>& value,
                                      Cb align) {
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_down(const poly_int_pod<N, Ca>& value,
                                        Cb align) {
    gcc_checking_assert(can_align_p(value, align));
    return value - (value.coeffs[0] & (align - 1));
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> aligned_lower_bound(const poly_int_pod<N, Ca>& value,
                                           Cb align) {
    poly_int<N, Ca> r;
    for (unsigned int i = 0; i < N; i++)
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> aligned_upper_bound(const poly_int_pod<N, Ca>& value,
                                           Cb align) {
    poly_int<N, Ca> r;
    for (unsigned int i = 0; i < N; i++)
        POLY_SET_COEFF(Ca, r, i,
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_down_and_div(
    const poly_int_pod<N, Ca>& value, Cb align) {
    gcc_checking_assert(can_align_p(value, align));
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_up_and_div(const poly_int_pod<N, Ca>& value,
                                              Cb align) {
    gcc_checking_assert(can_align_p(value, align));
...
inline bool known_misalignment(const poly_int_pod<N, Ca>& value, Cb align,
                               Cm* misalign) {
...
inline POLY_BINARY_COEFF(Ca, Ca) known_alignment(const poly_int_pod<N, Ca>& a) {
    typedef POLY_BINARY_COEFF(Ca, Ca) C;
    C r = a.coeffs[0];
    for (unsigned int i = 1; i < N; ++i) r |= a.coeffs[i];
    return r & -r;
}
...
inline typename if_nonpoly<Cb, bool>::type can_ior_p(
    const poly_int_pod<N, Ca>& a, Cb b, Cr* result) {
...
template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline typename if_nonpoly<Cb, bool>::type constant_multiple_p(
    const poly_int_pod<N, Ca>& a, Cb b, Cm* multiple) {
    typedef POLY_CAST(Ca, Cb) NCa;
    typedef POLY_CAST(Cb, Ca) NCb;

    /* Do the modulus before the constant check, to catch divide by
       zero errors.  */
    if (NCa(a.coeffs[0]) % NCb(b) != 0 || !a.is_constant()) return false;
    *multiple = NCa(a.coeffs[0]) / NCb(b);
    return true;
}

template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline typename if_nonpoly<Ca, bool>::type constant_multiple_p(
    Ca a, const poly_int_pod<N, Cb>& b, Cm* multiple) {
    typedef POLY_CAST(Ca, Cb) NCa;
    typedef POLY_CAST(Cb, Ca) NCb;
    typedef POLY_INT_TYPE(Ca) int_type;
...
template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline bool constant_multiple_p(const poly_int_pod<N, Ca>& a,
    ...
    if (NCa(a.coeffs[0]) % NCb(b.coeffs[0]) != 0) return false;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type multiple_p(
    const poly_int_pod<N, Ca>& a, Cb b) {
    for (unsigned int i = 0; i < N; ++i)
        if (a.coeffs[i] % b != 0) return false;
    return true;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type multiple_p(
    Ca a, const poly_int_pod<N, Cb>& b) {
    typedef POLY_INT_TYPE(Ca) int_type;
...
template <unsigned int N, typename Ca, typename Cb>
inline bool multiple_p(const poly_int_pod<N, Ca>& a,
                       const poly_int_pod<N, Cb>& b) {
    if (b.is_constant()) return multiple_p(a, b.coeffs[0]);
...
template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline typename if_nonpoly<Cb, bool>::type multiple_p(
    const poly_int_pod<N, Ca>& a, Cb b, poly_int_pod<N, Cm>* multiple) {
    if (!multiple_p(a, b)) return false;
    for (unsigned int i = 0; i < N; ++i) multiple->coeffs[i] = a.coeffs[i] / b;
    return true;
}
...
inline typename if_nonpoly<Ca, bool>::type multiple_p(
    ...
    /* Do the modulus before the constant check, to catch divide by
       potential zeros.  */
    if (a % b.coeffs[0] != 0 || (NCa(a) != 0 && !b.is_constant())) return false;
...
inline bool multiple_p(const poly_int_pod<N, Ca>& a,
    ...
    if (b.is_constant()) return multiple_p(a, b.coeffs[0], multiple);
...
inline POLY_POLY_RESULT(N, Ca, Cb)
    ...
    if (b.is_constant()) return exact_div(a, b.coeffs[0]);
...
inline typename if_nonpoly2<Cb, Cq, bool>::type can_div_trunc_p(
    const poly_int_pod<N, Ca>& a, Cb b, Cq* quotient) {
    typedef POLY_CAST(Ca, Cb) NCa;
    typedef POLY_CAST(Cb, Ca) NCb;
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline typename if_nonpoly<Cq, bool>::type can_div_trunc_p(
    const poly_int_pod<N, Ca>& a, const poly_int_pod<N, Cb>& b, Cq* quotient) {
    ...
    for (unsigned int i = 1; i < N; ++i) {
        if (b.coeffs[i] == ICb(0)) {
            ...
            if (a.coeffs[i] != ICa(0)) return false;
        } else {
            if (q == 0) {
                ...
                if (a.coeffs[i] != ICa(0)) {
                    ...
                    if (neg_abs_a < neg_abs_b) return false;
                    rem_p = true;
                }
            } else {
                /* Otherwise just check for the case in which ai / bi == Q.  */
                if (NCa(a.coeffs[i]) / NCb(b.coeffs[i]) != q) return false;
                if (NCa(a.coeffs[i]) % NCb(b.coeffs[i]) != 0) rem_p = true;
            }
        }
    }
    ...
    /* If the division isn't exact, require both values to be ordered wrt 0,
       ...
       values.  */
    if (rem_p && (!ordered_p(a, ICa(0)) || !ordered_p(b, ICb(0)))) return false;
...
inline typename if_nonpoly<Cq, bool>::type can_div_trunc_p(
    const poly_int_pod<N, Ca>& a, const poly_int_pod<N, Cb>& b, Cq* quotient,
    Cr* remainder) {
    if (!can_div_trunc_p(a, b, quotient)) return false;
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline typename if_nonpoly<Cb, bool>::type can_div_trunc_p(
    const poly_int_pod<N, Ca>& a, Cb b, poly_int_pod<N, Cq>* quotient) {
    /* The remainder must be constant.  */
    for (unsigned int i = 1; i < N; ++i)
        if (a.coeffs[i] % b != 0) return false;
    for (unsigned int i = 0; i < N; ++i) quotient->coeffs[i] = a.coeffs[i] / b;
    return true;
}
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline typename if_nonpoly<Cq, bool>::type can_div_away_from_zero_p(
    const poly_int_pod<N, Ca>& a, const poly_int_pod<N, Cb>& b, Cq* quotient) {
    if (!can_div_trunc_p(a, b, quotient)) return false;
    if (maybe_ne(*quotient * b, a)) *quotient += (*quotient < 0 ? -1 : 1);
    return true;
}
...
template <unsigned int N, typename C>
void print_dec(const poly_int_pod<N, C>& value, FILE* file) {
...
struct poly_span_traits<T1, T2, unsigned HOST_WIDE_INT> {
    template <typename T>
    static typename if_nonpoly<T, unsigned HOST_WIDE_INT>::type cast(
        const T& x) {
        return x;
    }

    template <unsigned int N, typename T>
    static poly_int<N, unsigned HOST_WIDE_INT> cast(
...
template <typename T>
inline bool known_size_p(const T& a) {
...
inline bool maybe_in_range_p(const T1& val, const T2& pos, const T3& size) {
    ...
    if (known_lt(val, pos)) return false;
    if (!known_size_p(size)) return true;
...
inline bool ranges_known_overlap_p(const T1& pos1, const T2& size1,
    ...
    /* known_gt (POS1 + SIZE1, POS2)                         [infinite
       precision]
       --> known_gt (SIZE1, POS2 - POS1)                     [infinite
       precision]
       --> known_gt (SIZE1, POS2 - lower_bound (POS1, POS2)) [infinite
       precision]
                            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ always nonnegative
...
template <typename T>
inline typename if_nonpoly<T, bool>::type endpoint_representable_p(
    const T& pos, const T& size) {
    return (!known_size_p(size) ||
            pos <= poly_coeff_traits<T>::max_value - size);
}

template <unsigned int N, typename C>
inline bool endpoint_representable_p(const poly_int_pod<N, C>& pos,
                                     const poly_int_pod<N, C>& size) {
    if (known_size_p(size))
        for (unsigned int i = 0; i < N; ++i)
            if (pos.coeffs[i] >
                poly_coeff_traits<C>::max_value - size.coeffs[i])
                return false;
    return true;
}

template <unsigned int N, typename C>
void gt_ggc_mx(poly_int_pod<N, C>*) {}

template <unsigned int N, typename C>
void gt_pch_nx(poly_int_pod<N, C>*) {}
