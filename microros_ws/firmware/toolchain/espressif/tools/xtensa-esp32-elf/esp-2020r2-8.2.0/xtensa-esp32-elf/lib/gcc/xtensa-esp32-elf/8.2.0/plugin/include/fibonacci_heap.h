...
class fibonacci_heap {
   ...
   public:
    ...
    /* Destructor.  */
    ~fibonacci_heap() {
        while (m_min != NULL) delete (extract_minimum_node());
    }
    ...
    bool empty() { return m_nodes == 0; }

    /* Return the number of nodes.  */
    size_t nodes() { return m_nodes; }

    /* Return minimal key presented in the heap.  */
    K min_key() {
        if (m_min == NULL) gcc_unreachable();
...
fibonacci_node<K, V>* fibonacci_node<K, V>::remove() {
    ...
    if (m_parent != NULL && m_parent->m_child == this) m_parent->m_child = ret;
...
template <class K, class V>
fibonacci_node<K, V>* fibonacci_heap<K, V>::insert_node(
    ...
    /* If their was no minimum, or this key is less than the min,
       it's the new min.  */
    if (m_min == NULL || node->m_key < m_min->m_key) m_min = node;
...
V* fibonacci_heap<K, V>::replace_key_data(fibonacci_node<K, V>* node, K key,
    ...
    if (okey == key && okey != m_global_min_key) return odata;
    ...
    if (node->compare(m_min) <= 0) m_min = node;
...
template <class K, class V>
V* fibonacci_heap<K, V>::extract_min(bool release) {
...
template <class K, class V>
fibonacci_heap<K, V>* fibonacci_heap<K, V>::union_with(
    fibonacci_heap<K, V>* heapb) {
    ...
    if (heapb->m_min->compare(heapa->m_min) < 0) heapa->m_min = heapb->m_min;
...
void fibonacci_heap<K, V>::consolidate() {
    ...
    for (i = 0; i < D; i++)
        if (a[i] != NULL) {
            insert_root(a[i]);
            if (m_min == NULL || a[i]->compare(m_min) < 0) m_min = a[i];
        }
}
